# -*- coding: utf-8 -*-
r"""门-值快照键型判模-课时09.py — M3 S3 W2臂B 课时09 练习件·值快照逐字零漂＋键型两档＋全线括线判模断言。
（照抄母版门谱 工作区/_tmpM3S3母版0914/门谱/门-值快照键型判模.py，PIECE/SIDE/MANIFEST/OUT/PROC
常量换片：本片 16 键答案侧值全为具体值（T15/T16 纯题无小问），PROC（「证明见详解．」过程型）＝空集。）
用法: python 门-值快照键型判模-课时09.py   退出码: 0＝全过；1＝有红。
红线：件树/题面库只读；唯一写件＝底稿-课时09.json（本过程件目录）。
"""
import fitz  # pymupdf
import io
import json
...
SIDE = 'C:/提示词/工作区/M3-第2章量产0913/成卷/题面库/课时09-圆与圆的位置关系-答案侧.md'
MANIFEST = 'C:/提示词/工作区/M3-第2章量产0913/成卷/题面库/manifest/课时09.manifest.json'
OUT = os.path.join(os.path.dirname(os.path.abspath(__file__)), '底稿-课时09.json')
