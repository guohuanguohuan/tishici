# -*- coding: utf-8 -*-
r"""门-值快照键型判模.py — M3 S2 波1 臂6 课时12·值快照逐字零漂＋全值型＋括线判模断言。
参数化自 母版 门谱，常量换片：PIECE/SIDE/NUM/21；课时12 无过程型键（21 键全值型）。
断言面：
  ...
  ③括线判模（承重墙判据，双证）：
    ...
    逐键估高读数落盘 值台账底稿-课时12.json（本过程件目录）。
用法: python 门-值快照键型判模.py
退出码: 0＝全过；1＝有红。红线：件树/题面库只读；唯一写件＝值台账底稿-课时12.json。
"""
import fitz  # pymupdf
import io
import json
...
OUT = os.path.join(os.path.dirname(os.path.abspath(__file__)), '值台账底稿-课时12.json')
NKEYS = 21
# 印面号映射（装配序连号 1–21＝探究1—16＋评价17—21，见 main.tex 头注；键尾段→号）
NUM = {'简1': 1, '简2': 2, '简3': 3, '简4': 4, '简5': 5, '简6': 6,
