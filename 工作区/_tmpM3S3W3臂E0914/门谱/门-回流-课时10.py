# -*- coding: utf-8 -*-
r"""门-回流-课时10.py — M3 S3 W3臂E 课时10 练习件·回流门（零强制跳页）＋件型纪律断言。
（照抄母版门谱 工作区/_tmpM3S3母版0914/门谱/门-回流.py，PIECE/MANIFEST 常量换片；
逐片期望值：\duoxuan＝manifest 多选数 0；\liubai＝解答题数 12（简1~6、9＝16mm，
简10＝32mm 三问、简7＝24mm 两问、中1~中4＝16mm，装配序 16×7＋32＋24＋16×3）；
\kongda＝1（难2 填空印答位）。）
用法: python 门-回流-课时10.py     退出码: 0＝全过；1＝有红。
"""
import io
import json
import os
...
MANIFEST = 'C:/提示词/工作区/M3-第2章量产0913/成卷/题面库/manifest/课时10.manifest.json'
...
check('\\liubai 计数＝解答题数 12（题面侧头标实录）', n_liubai == 12, f'{n_liubai}')
inband = all(16.0 <= float(h[:-2]) <= 40.0 for h in liubai_h)
check('\\liubai 逐题高度 16~40mm 带内（16×7＋32 三问＋24 两问＋16×3 装配序）',
      inband and liubai_h == ['16mm'] * 7 + ['32mm', '24mm'] + ['16mm'] * 3, f'{liubai_h}')

n_kongda = len(re.findall(r'\\kongda(?![a-zA-Z])', body))
check('\\kongda 计数＝1（难2 填空印答位）', n_kongda == 1, f'{n_kongda}')
