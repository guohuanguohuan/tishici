...
% 图债登记：E1（国旗五星）题面「如图」占位保留，图工位补图后回冲印面（题面侧§六.1 在案）。
...
% ans:2章-练-课时02-E1
\ansitem{1}{C}
\ansnote{详解}{\(O\)、\(O_3\) 都为五角星的中心点，故 \(OO_3\) 平分第三颗小星的一个角；又五角星的内角为 \(36^{\circ}\)，可知\(\angle BAO_3=18^{\circ}\)．过 \(O_3\) 作 \(x\) 轴的平行线 \(O_3E\)，则\(\angle OO_3E=\alpha\approx 16^{\circ}\)，所以直线 \(AB\) 的倾斜角约为 \(18^{\circ}-16^{\circ}=2^{\circ}\)，选C．（图债在途：本题解算不依赖图，补图后回冲印面．）}
\end{ansblock}
