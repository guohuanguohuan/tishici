...
GATES = [
    ...
    dict(id='钉-3', key='导-课时03-探四例1',
         want='B', rollback='120',
         note='探究四 例1 答案值钉 B；原登记「讲部详解 C 项夹角 120° 与亲算不符（cos=−√2/4）」系 S2 亲算误算'
              '（|B₁C| 误取 6√2、漏计 ⟨b,c⟩=60°）——2026-09-12 S7收尾批撤单：复算 |B₁C|=6、cos=−1/2、'
              '夹角恰 120°，讲部详解正确，改字义务注销（义务清单义2-1 撤销；导学-课1-3-glm.md 键离1）；'
              '负测串「120」维持——拦角值文字渗入字母键答案位',
    ...
    # ---- S7 修键·题面改正0913 追加（钉-18，导学件课09 探五变式1 折叠题，裁「改题面不改键」）----
    # 键式为导学件聚合槽 导-课时09-探5-变式1：不触拓号集、不触钉-6（拓-057 哨兵）与钉-7；--拓159 对号档正交。
    # 裁断＝命制原件（批5值台账 §1.3 亲算 √6，BD=DC=√3）为准，误在卷面题面「边长为2」→已改正「边长为 2√3」；
    # 键值 √6 与答案册/值快照零改动。本门锁答案 √6，rollback＝误按边长 2 版会得之 √2（灌回必拦·防回潮）。
    dict(id='钉-18', key='导-课时09-探5-变式1',
         want=r'\sqrt{6}', rollback=r'\sqrt{2}',
         note='S7 题面改正0913 值钉（边长 2√3 正三角形沿高 AD 折成直二面角 B-AD-C ⟹ BD=DC=√3、∠BDC=90° ⟹ '
              '折后 BC=√(3+3)=√6，与命制原件/答案册 √6 一致；旧卷面「边长为2」会导出 √2——键无错、错在题面，'
              '题面已改 2→2√3；灌 √2 必拦，防「改错方向」回潮）',
         src='逻辑闸/导学-课9-10-qwen.md 键离1；逻辑闸/修键-课09探五变式1-qwen.md', status='生效'),
    # ---- 目验修红0913 追加（钉-19，拓展册拓-144 双空答案全串钉）----
    # 键式为拓号：拓-144 在在产 159 档内（撤号哨兵只盯拓-057），不触钉-6；--拓159 对号档正交。
    # 目验红旗「拓-144 答案缺范围」经核＝**双空题误报**（两空俱在：直线情形 m=1 ＋圆情形 0<m<√2 且 m≠1，
    # 卷面/值台账/组装器内嵌表三处同值）→ 内容零改动，改以本门钉**全串**防「只印第一空」半答案回潮。
    dict(id='钉-19', key='拓-144',
         want=r'\(m=1\)；\(0<m<\sqrt{2}\) 且 \(m\neq 1\)', rollback=r'\(m=1\)',
         note='目验修红0913 双空答案全串钉（拓-144 轨迹题两空＝直线 m=1 ＋圆 0<m<√2 且 m≠1；红旗经核系双空题'
              '误报，值零改动）。钉全串：缺范围旧制（仅印第一空 m=1）灌回必拦——半答案即错答案',
         src='拓展册/值台账.md:154；逻辑闸/拓展-下2-qwen.md:89', status='生效'),
]
