...
\par\vspace{1.9mm}\penalty10000\noindent\makebox[\linewidth][c]{\includegraphics[width=28mm]{figs/dy7_image2.png}}\par\vspace{-1.0mm}\penalty10000
...
\tihao{4}\zu{K3 定点到定直线段距离×正三角形中心垂线} \tieside{简单}如图所示，已知正三角形\(ABC\)的中心为\(O\)，\(OP\perp\)平面\(ABC\)且\(AB=2OP=2\,\mathrm{cm}\)，求点\(P\)到这个正三角形各边的距离．
\xiexwei{20mm}
...
\par\vspace{1.9mm}\penalty10000\noindent\makebox[\linewidth][c]{\includegraphics[width=25mm]{figs/jx_image127.png}}\par\vspace{-1.0mm}\penalty10000
...
\par\vspace{1.9mm}\penalty10000\noindent\makebox[\linewidth][c]{\includegraphics[width=32mm]{figs/jx_image151.png}}\par\vspace{-1.0mm}\penalty10000
...
\tihao{13}\zu{异面直线所成角×坐标法三小问×解答（C组难档槽位，亲判简单偏中档）} \tieside{难}如图所示，已知四棱锥\(P\text{-}\penalty100 ABCD\)中，\(ABCD\)为矩形，\(PD\perp\)平面\(ABCD\)，\(PD=3\)，\(AB=5\)，\(BC=4\)，求下列各对异面直线所成角的余弦值：
\lxopt{(1)\(PC\)与\(AB\)；}
\lxopt{(2)\(PD\)与\(AB\)；}
\lxopt{(3)\(PA\)与\(BC\)．}
\xiexwei{64mm}

\tihao{14}\zu{线面角×定比分点×三垂线综合×解答（C组难档槽位，亲判中档偏上）} \tieside{难}如图所示，直三棱柱\(ABC\text{-}\penalty100 A_1B_1C_1\)中，\(AC\perp BC\)，点\(M\)在线段\(AB\)上，\(AC=BC=CC_1=3\)，\(AM=\sqrt{2}\)，求直线\(AC_1\)与平面\(B_1MC\)所成角的正弦值．
\xiexwei{84mm}
