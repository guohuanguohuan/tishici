# -*- coding: utf-8 -*-
r"""门-值快照键型判模18.py — M3 S2 波1 补位臂4b·课时18·值快照逐字零漂＋键型两档＋括线判模断言。
适配自臂4 门谱（课时15 版）：PIECE/SIDE/MANI/OUT→课时18；NKEY=21（全值型，无过程键）；
NUM＝印面装配序全键映射（探究 1—16＋课堂评价 17—21）；RULED 载 括线键-课时18.json（首轮 [] 跑读数）。
唯一写件＝值台账底稿-课时18.json（本目录）。
"""
import fitz  # pymupdf
import io
import json
...
SIDE = 'C:/提示词/工作区/M3-第2章量产0913/成卷/题面库/课时18-2.8②压轴综合二-答案侧.md'
MANI = 'C:/提示词/工作区/M3-第2章量产0913/成卷/题面库/manifest/课时18.manifest.json'
OUT = os.path.join(os.path.dirname(os.path.abspath(__file__)), '值台账底稿-课时18.json')

PROC = set()
RULED = set(json.load(open(os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                        '括线键-课时18.json'), encoding='utf-8')))
NRULE = len(RULED)
NKEY = 21
NUM = ('2章-练-课时18-01:1,2章-练-课时18-07:2,2章-练-课时18-08:3,2章-练-课时18-10:4,'
       '2章-练-课时18-11:5,2章-练-课时18-09:6,2章-练-课时18-03:7,2章-练-课时18-04:8,'
       '2章-练-课时18-05:9,2章-练-课时18-06:10,2章-练-课时18-15:11,2章-练-课时18-02:12,'
       '2章-练-课时18-12:13,2章-练-课时18-13:14,2章-练-课时18-14:15,2章-练-课时18-16:16,'
       '2章-导-课时18-G1:17,2章-导-课时18-G2:18,2章-导-课时18-G3:19,'
       '2章-导-课时18-G4:20,2章-导-课时18-G5:21')
