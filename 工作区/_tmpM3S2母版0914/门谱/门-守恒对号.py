# -*- coding: utf-8 -*-
r"""门-守恒对号.py — M3 S2 课时01 母版·答案块守恒断言（双档）＋对号门＋尾块门＋页数门。

断言面：
  ①tex 层：行首注释锚 % ans:键 ≡ ansblock[键] ≡ manifest 键序（23 键，集合双向 diff＋逐位序）。
  ②log 层：M3-ANSKEY 键序（true/false 两档各 23）≡ manifest 键序（逐位相等）。
  ③PDF 层（fitz 文本抽取）：
      true ：「N. [答案]」号序 ≡ 1..23 严格连号（对号门）；[答案] 总计 23；
...
MANIFEST = 'C:/提示词/工作区/M3-第2章量产0913/成卷/题面库/manifest/课时01.manifest.json'
...
check('锚数=23', len(anchors) == 23, f'{len(anchors)}')
check('块数=23', len(blocks) == 23, f'{len(blocks)}')
check('锚集合≡manifest键集合(双向diff)', set(anchors) == set(keyseq) and len(set(anchors)) == 23,
...
check('ansitem 号序≡1..23 连号', nums == list(range(1, 24)), f'{nums if len(nums) != 23 else "23枚"}')
note_cnt = len(RE_NOTE.findall(src))
check('ansnote{详解} 计数>0 且=23 键均有详解行', note_cnt == 23, f'{note_cnt}')
...
check('true 号序≡1..23 连号(对号门)', tnums == list(range(1, 24)), f'{len(tnums)}枚')
check('true [答案] 总计=23', pdf['true']['text'].count('[答案]') == 23,
