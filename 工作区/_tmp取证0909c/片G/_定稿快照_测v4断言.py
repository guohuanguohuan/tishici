...
# FIG_W＝body.tex 图行阅读序（＝\input{figs/…} 序）＋tex 声明置宽；BOX_MM＝\linewidth 折 84.0mm；
# FIG_AT＝预期落位（页, 栏）——固定版式件，落位推移即 N6 报错。
# 矢量簇按（页, 栏, y 顶）排序即阅读序：下置图行各自成段，twocolumn 流序与片段序严格一致
# （旧并排口径下图矩形与文字段同带，此式不成立——片G 起成立）。
FIG_W = [('g6-triple', '\\linewidth'), ('g1-prism', '41.1mm'), ('g2-cubeE', '42.0mm'),
         ('g3-cube6', '\\linewidth'), ('g4-dihedral', '66.3mm'), ('g5-fold', '54.7mm')]
...
#      六处逐字节同式，只差片段名与置宽；置宽档逐图钉死（\linewidth＝84.0mm 通栏档）——FIG_W 见共用台账。
...
# 门值由权威实测直读（run4-权威实测.txt【七】，窗＝实测极值±0.25mm 墨级安全垫）：前距 含标 [1.99,4.14]；
# 下距 含标 [2.04,4.36]；栏心偏 线框基 |·|≤1.45；Δ线框（件内−素材×k）有画布锁定五图 ≤0.50mm；
# 无锁定片段（g5）不适用同比参考系 → 改核不变量（|rx−ry|≤0.005＋形状比差≤0.005＋|含标宽−置宽|≤0.30mm）；
# 硬门：正文相交=0／越声明盒≤0.30mm／标签数件内==素材／虚线保真率≥0.99。
...
for (_p, _c01, _y, _clus), (tag, srcf, boxmm, frag, at) in zip(_kfigs, K_MAP):
    ...
    oks = [('前距', gap_u is not None and 1.99 <= gap_u <= 4.14),
           ('下距', gap_d is not None and 2.04 <= gap_d <= 4.36),
           ('栏心偏', abs(dev) <= 1.45),
           ('零侵入', len(m['hits']) == 0),
           ('越盒', over <= 0.30),
           ('标签数', m['nlab'] == ms['nlab']),
...
check('⑱-2 六图线框几何（前距 1.99–4.14／下距 2.04–4.36／栏心偏 ±1.45／Δ 或 g5 不变量／零侵入=0）',
...
reg('N6 口径登记（片G 0910 旧→新）',
    ...
    'g3@p5c1／g4@p6c1／g5@p6c2；置宽档由 ⑱ tex 形制门核（84.0/41.1/42.0/84.0/66.3/54.7mm）。')
