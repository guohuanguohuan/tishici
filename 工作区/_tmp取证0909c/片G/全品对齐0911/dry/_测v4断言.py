...
check('⑥排印层 正文判分值 0 在场（答案制0911）◆9/例1 9/变式9 题面不变＋[答案]0/故答案为0/故选0/分析0/详解0/点睛0/解析6(仅判断)',
      n_tj == 9 and n_li1 == 9 and n_bs == 9 and n_ans == 0 and n_star == 0 and n_jx == 6
      ...
      f' 故答案{n_gdaan} 故选{n_gxuan}（[解析]6＝判断题简析×6，题面/选项/填空位保留；值在场见 ㉑b/㉑c）')
...
check('⑰解析 10.5pt 档（span 10–11 ×6＝判断题简析保留；答案制0911 变式/检测简析 14→出册）＋\\zhuzhu [注意]前缀新体无小号层',
      sz_ok and n_jx_span == 6 and no_small,
      f'解析span {n_jx_span}/6 越档{0 if sz_ok else "有"}；\\zhuzhu 新体 {"是" if no_small else "否"}')
...
check('⑦ 解析→下题题号墨距（答案制0911 重释：正文简析仅存判断域——配对 n 1–2 且逐对 ∈2.0–3.5mm（判断缝域，⑦-4 同族）；旧门 n≥3 中位5.21±0.5 随变式/检测简析 14 行出册）',
      1 <= len(gaps_bb) <= 2 and all(2.0 <= g <= 3.5 for g in gaps_bb),
...
check('⑫判断题 zhenti ×6（保留）＋简析 jiexi ×0（答案制0911：变式9＋检测5 简析 14 行移出正文、入册）',
      n_zt == 6 and n_jx_body == 0,
...
# 正文侧「判分值 0 在场」＝⑥（含故答案为/故选）＋⑩ 反向门＋⑫ jiexi 0；分值保留面＝④(√)(×)×6＋⑪ kongda×20。
# 册侧「值在场」：编译三0＋页数4；排印七项计数；23 题（题号+[答案]+值核）逐一 pdf 明文包含判定。
import unicodedata
...
cnts21 = {'[答案]': 23, '[分析]': 9, '[详解]': 9, '[点睛]': 2, '[解析]': 14, '题型:': 18, '[题型总结]': 9}
# （atxt 过 NFKC——全角冒号折半角，键用 ASCII '题型:'）
miss21 = [f'{t}={atxt.count(t)}/{w}' for t, w in cnts21.items() if atxt.count(t) != w]
check('㉑b 答案册排印计数（pdf 明文去空白：[答案]23/分析9/详解9/点睛2/解析14/题型行18/题型总结9）＋位图2',
