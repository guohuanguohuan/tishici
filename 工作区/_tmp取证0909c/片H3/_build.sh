#!/bin/bash
# 片H3 构建＋三0计数：用法 bash _build.sh <档名>
# 产出：variantF/main.pdf（＋快照 _tmp取证0909c/片H3/档_<档名>.pdf）
set -u
VF="C:/提示词/工作区/字替对照-0909/variantF"
OUT="C:/提示词/工作区/_tmp取证0909c/片H3"
TAG="${1:-run}"
cd "$VF" || exit 1
echo "=== [$TAG] postproc ==="
...
echo "=== [$TAG] 三0计数 ==="
e=$(grep -c "^! " main.log)
ovh=$(grep -c 'Overfull \\hbox' main.log); ovv=$(grep -c 'Overfull \\vbox' main.log)
unh=$(grep -c 'Underfull \\hbox' main.log); unv=$(grep -c 'Underfull \\vbox' main.log)
ov=$((ovh + ovv)); un=$((unh + unv))
