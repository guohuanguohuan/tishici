# -*- coding: utf-8 -*-
r"""门-守恒对号.py — M3 S2 课时17·答案块守恒断言（双档）＋对号门＋页数门（波1补位臂4a）。

断言面（母版同构，50→42 键）：
  ①tex 层：行首注释锚 % ans:键 ≡ ansblock[键] ≡ manifest 键集合（42 键，集合双向 diff＋装配序一致）。
  ②log 层：M3-ANSKEY 键序（true/false 两档各 42）集合≡manifest；两档逐位同序。
  ③PDF 层（fitz 文本抽取）：
      true ：「N. [答案]」号序 ≡ 1..42 严格连号（对号门）；[答案] 总计 42；
...
def pdf_norm(text):
    """抽取归一化（注册项，非判据放宽）：\\anlabel 加 \\mbox 壳后（拓17-04/16 标签原子化），
    fitz 抽取在字体 span 边界（[ ↔ 答）插空格／断行（[ 答案]、[答\\n案]）。印面目验 0914：
    标签样式与 [详解] 一致无裂距。门比对前将 [答案]/[详解] 标签内空白折叠。"""
    text = re.sub(r'\[\s*答\s*案\s*\]', '[答案]', text)
    text = re.sub(r'\[\s*详\s*解\s*\]', '[详解]', text)
    return text
...
check(f'ansitem 号序≡1..{NK} 连号', nums == list(range(1, NK + 1)), f'{nums if len(nums) != NK else "42枚"}')
...
for tag in ('true', 'false'):
    ...
    pdf[tag] = {'n': len(doc), 'text': pdf_norm('\n'.join(p.get_text() for p in doc))}
    doc.close()
