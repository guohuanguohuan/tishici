% ============================================================
% 【物理骨架 0911｜继承件＋尾部增段】§ZS…TS 52 零件参数零改动，来源＝数学 L1
%   （工作区/字替对照-0909/靠齐样张-0910/测评卷/qp-parts.tex，md5 ca28db7d…）。
%   物理化增段＝文件尾「TW-RD 读数刻度图显示宽档（物理新设·待拍板）」，不动既有任何宏。
% ============================================================
...
% ---------------- TW-RD 读数刻度图显示宽档（物理新设 0911·待拍板） ----------------
% 立项盘点 §二 A2＋§六-2：游标卡尺/螺旋测微器/电表表盘刻度图，刻度可读是硬需求，
%   「图内标签≈7.5pt」判据对刻度线不适用——须定最小显示宽。样本源＝必修3 第11章实验卷
%   题 1–2 读数刻度图（元素 13–29）。候选三档 30/40/50mm 已上参数试样页（骨架/试样/）
%   目验对照；用户拍板后终值回填 \qpRDWdef 并升入《附则/全品零件库》TW 系编号。
%   【2026-09-11 用户拍板·按主脑推荐】终值＝下限 50mm，刻度可辨优先于紧凑、
%   不足即放宽至通栏 84mm（\qpfigsetw 档）；30/40mm 候选退役。
% 消费方＝qp-figs.tex \qpfigrd（低于下限编译告警）。
\newcommand{\qpRDWdef}{50}       % mm——拍板终值下限（试样页 C 档全可读）
\newcommand{\qpRDWloCand}{30}    % 候选低档（试样页 A）
\newcommand{\qpRDWmidCand}{40}   % 候选中档（试样页 B）
\newcommand{\qpRDWhiCand}{50}    % 候选高档（试样页 C）
