% ============================================================
% 【物理骨架 0911｜零参数继承件】本文件＝数学 L1 原样继承，未改任何块宏。
%   来源＝工作区/字替对照-0909/variantF/qp-blocks.tex（md5 3279de92…，含 0911 全品对齐轮
%   \zhentib 判断题正文式——比 靠齐样张-0910/测评卷 拷贝新 10 行，取 variantF 为权威源）。
%   答案制A（正文不印答案/解析，答案单独成册）随 \zhentib/无 \ansline 发射口径继承；
%   物理图族宏独立成件＝qp-figs.tex（位图引用），不并入本件。
% ============================================================
...
%   【解析】行后固定间距：\zhenti 尾 0pt（F 片B 0909 #31 归零）／\jiexi 尾 7pt（6–8pt 档，变式·检测语境维持）。
% 调用关系：main.tex → qp-titles → 本模块（body.tex/chapterhead.tex 使用这些宏）。
% ============================================================
\usepackage{tikz}
\usetikzlibrary{shapes.geometric}
% ---- 片G 0910 六图下置集成：素材自带宏随导言（figs/*.tikz 依赖，原先在各自 standalone 壳内）----
% g1-prism（探二例1 直三棱柱）标签宏：主体＝斜体数学体；下标＝源图式小号（9.68pt、下沉 0.649mm，按 41.1mm 基准标定）
...
\newcommand{\qlab}[1]{$#1$}
% g3-cube6（探六例1 正方体）灰棱色：源灰墨 #404040
...
% H1 片 0909c（拉伸异常行连带）：两侧 0.15em 由刚性改带弹性 plus 0.35em／minus 0.05em——
%   挖空盒＝不可断 hbox、内部胶不参与行拉伸：含挖空盒的行可用拉伸胶被压缩到「盒外少数几副」
%   （表1 共线行实测仅 2 副可拉 → 各超拉 5.47pt＝0.52em 触发断言）；侧隙入弹性后同线超拉
%   4 副分摊 → 归普通档。natural 0.15em 不动（不拉伸时视觉零变化），minus 0.05em 供拥塞收紧。
\newlength{\kdmind}
\newlength{\kdwd}
\newcommand{\kongda}[1]{{\setlength{\kdmind}{15mm}\settowidth{\kdwd}{#1}%
  \ifdim\kdwd>\kdmind \kdmind=\kdwd\fi
  \hspace{0.15em plus 0.35em minus 0.05em}\underline{\makebox[\kdmind][c]{#1}}\hspace{0.15em plus 0.35em minus 0.05em}}}
...
% H1 片 0909c（② 条目号后隙，全品 p04-p07 复测 n=30）：全品「N.」→内容 ink 3.10–3.39mm（中位 3.31，
%   均值 3.33；学习目标位 3.17 同档）；我方 \hspace{1em} 实测 3.877–3.920（偏宽 ≈0.56）。改固定
%   \kern8.5pt（＝2.998mm；ink＝kern＋'.'rsb＋首字 lsb ≈3.32 落窗）——kern 非断点，同片F 判断题
%   序号做法（\kern2.1pt）：零拉伸且「N.」不再可能悬行尾。注：任务书「全品 0.99mm 档」系与判断题
%   序号（(N) 后隙）串档——全品 (N) 后隙实测 0.87–0.92mm（p04 (1) 零向量行），本条按条目号复测档落。
% 尾距 knob（#15/#36：条目→条目 全品墨隙 3.06±0.2；F 片B 0909 复标：1.8→0.35mm——1.8mm 时
% 墨隙 4.49–4.53 超全品 1.4；0.35 落 3.0–3.4，基线 18.17+0.99pt≈6.8mm 档）
\newcommand{\tiaomucore}[2]{\par\glueguard{1}\noindent{\heitiao {\numbold #1.}}\kern8.5pt#2\par}
\newcommand{\tiaomu}[2]{\tiaomucore{#1}{#2}\addvspace{0.35mm}}
% 拆段条目（v4.2-C11：条目号段＋\bindp 续段）首段用零尾距变体——#28 修：1.8mm 尾距曾落 (1)→(2)
% 中缝致 8.21mm/墨 4.53（全品同段连排 6.44/2.7–3.0）；条目尾距改由 \tiaomutail 挂到条目末段（#36 条目缝统一）
\newcommand{\tiaomuz}[2]{\tiaomucore{#1}{#2}\addvspace{0mm}}
\newcommand{\tiaomutail}{\par\addvspace{0.35mm}}
...
% 花形前距（0908 渲染实测回填，ink 口径）：宏胶 0.9mm→3.37mm（F 片B 0909 #31 连带复标：\zhenti 解析尾
% 7pt→0 后考点探究（p3）前距原靠「7pt＋宏胶 0.9mm 叠加（\glueguard 重置 \lastskip 致加而非 max）」达
% 5.02，归零后须由宏胶独担——3.37mm＝7pt＋0.9mm 全额回填，实测仍 5.0 档；课前预习（p1）/知识评价（p7）
% 为栏首元素，行首胶被弃（p1 前距由 postproc LAN_OPEN 栏区外 \vspace{1.7mm} 给，p7 栏顶豁免登记）。
\newcommand{\huaxing}[5]{\par\glueguard{0}\addvspace{3.37mm}\noindent
...
% 例N 标签维持 \li（方圆 w600）；变式标签 \liB 见下（F 片D 0909c 双字重）。
...
% F 片D 0909c 变式标签双字重（意见#33）：全品 p06 真迹＝「变」特黑 0.145em/8.7px＋「式」中黑 0.075em/4.5px——
% 拆两段各自挂族：\liBsplit 以 \liBstop 为界取首字「变」→ \libianb（FY-w700，标定 7.08px）；余部「式N」→
% \libians\libianslat（FY-w450，标定 4.39px，数字随同族西文）。E 版整串 w400（3.8px）退役：4px 靶只量「式」漏「变」。
% F 片F 0909c #41：两族接缝 \kern0pt——xeCJK 见用户 kern 即不再插 CJKglue（_calib 探针 \showbox 实证：
%   现状 V0 接缝 \glue 0.0 plus 0.41786，V1 kern 后胶节点消失），「变|式」字间不再随行拉伸浮动
%   （改前 p3 变式1 行实测 6.18pt＝0.51em 空档，改后应归 0；设计字距 0pt 不动，探针 lab.log）。
% H1 片 0909c（③ 变式标签行残余）：同理再插「式|数字」接缝——数字原随 xeCJK ecglue（0.25em＋拉伸）
%   浮动：改前四实例 ink 11.46/7.93/10.68/9.67pt（>0.5em 限）。固定档取 kern2.5pt（＝ecglue 自然值
%   2.5225pt 复刻）而非 0：零拉伸达成（kern 非胶、非断点），且标签自然宽与基线一致——0pt 档实测
%   连带全件重排（标签窄 2.5pt×9 → 末页 monolith 跳出、p6 右栏空 63%），kern2.5 保流水同基线。
%   拆法：\liBsplitb 再取首字「式」；数字随后随 \libianslat（FY-w450）。
\def\liBsplit#1#2\liBstop{{\libianb #1}\kern0pt\liBsplitb#2\liBstop}
\def\liBsplitb#1#2\liBstop{{\libians #1}\kern2.5pt{\libianslat #2}}
...
% F 片B 0909（#31 相邻判断题缝）：7pt→0pt——原相邻题缝 8.88mm/墨 5.38，超全品 6.3–6.8/≈2.9
% 达 7pt＝2.47mm；归零后缝＝普通行距档 6.44mm/墨≈2.9（\jiexi 同挂 7pt 但语境为变式/检测，勿动）。
...
% F 片F 0909c #43（序号间隙固定化）：#1#2→#1\kern2.1pt#2——原「(N)」后走 xeCJK ecglue（空格
%   2.5225pt plus 1.26125），随行拉伸浮动（基线 6 处 ink 实测 1.094–4.239mm，随该行拉伸走）；
%   改显式 \kern：xeCJK 见用户 kern 不再插 ecglue（_calib/探_序号隙.tex \showbox 实证 W0 有胶、
%   W1 无胶），间隙钉 2.1pt（＝0.99mm 靶反解：ink＝adv＋')'rsb0.43＋首字 lsb0.16–0.52＝
%   0.95–1.08mm，落 0.99±0.1 窗），零拉伸且顺带禁断（kern 非断点，序号不再可能悬行尾）。
%   只作用本宏一处，正文其它 CJK–西文边界（ecglue）不动。
\newcommand{\zhenti}[4]{\par\glueguard{1}%
  ...
  \nopagebreak\noindent{\anlabel{[解析]}\hspace{0.5em}#4}\par\addvspace{0pt}}
% 全品对齐0911 \zhentib：判断题正文式——全品 p04 实拍（工作区/全品结构提取/数学选必一/导学案页图/）
% 题干行尾仅右挂全角空括号（　），不印答案与解析（答案单独立册）。右挂机制承 \zhenti 同款
% （fil 大胶＋\kern2.1pt 序号隙＋尾 \hspace{0.56mm}＋\addvspace{0pt} 同档）。
% v10-A 断行优化（对齐0911）：旧形「句尾↔（　）」双 \nobreak 禁断链致题干超一行时整块「等.（　）」
% 挤次行、行1 尾余空 33.2pt（≈3.4 字）。新形＝句尾后开一个廉价断点（\penalty0），断点与 fil 胶之间
% 垫 \hbox{}＋\nobreak 封死「断在胶上丢胶」支路——题干完整收行，（　）独落次行仍由 fil 右对齐贴栏。
% 发射方＝postproc 6h（\zhenti→\zhentib 6 处，生成链可重跑）。④/⑥/N1 正文侧锁本宏形。
\newcommand{\zhentib}[2]{\par\glueguard{1}%
  {\rightskip=0pt plus 1fil\parfillskip=0pt\emergencystretch=3em\noindent#1\kern2.1pt#2\penalty0\hbox{}\nobreak\hspace{0pt plus 1000fil}\nobreak（　）\hspace{0.56mm}\par}%
  \addvspace{0pt}}
