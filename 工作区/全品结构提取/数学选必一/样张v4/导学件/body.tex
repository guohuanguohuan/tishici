...
\noindent\begin{tabular}{|>{\raggedright\arraybackslash}p{16mm}|>{\raggedright\arraybackslash}p{35mm}|>{\raggedright\arraybackslash}p{29mm}|}
\hline
\multicolumn{1}{|>{\centering\arraybackslash}p{16mm}|}{\textbf{名称}} & \multicolumn{1}{|>{\centering\arraybackslash}p{35mm}|}{\textbf{定义}} & \multicolumn{1}{|>{\centering\arraybackslash}p{29mm}|}{\textbf{表示}} \\
\hline
...
相等向量 & 方向相同且\kongda{模相等}的向量 & 记作 \(\overrightarrow{a}=\overrightarrow{b}\) \\ \hline
\end{tabular}\par
}
...
\noindent\begin{tabular}{|>{\raggedright\arraybackslash}p{10mm}|>{\raggedright\arraybackslash}p{37mm}|>{\raggedright\arraybackslash}p{32mm}|}
\hline
\multicolumn{1}{|>{\centering\arraybackslash}p{10mm}|}{\textbf{运算}} & \multicolumn{1}{|>{\centering\arraybackslash}p{37mm}|}{\textbf{法则要点}} & \multicolumn{1}{|>{\centering\arraybackslash}p{32mm}|}{\textbf{运算律举例}} \\
\hline
...
\bindp \penalty10000\noindent\makebox[\linewidth][c]{\includegraphics[width=45mm,alt={@@@5da5e1ea-9d33-4579-a56f-804b4a72c894}]{media/media/sub3_B_4.png}}\par\penalty10000
...
{\setlength{\tabcolsep}{3pt}
\noindent\begin{tabular}{|>{\raggedright\arraybackslash}p{10mm}|>{\raggedright\arraybackslash}p{36mm}|>{\raggedright\arraybackslash}p{33mm}|}
\hline
\multicolumn{1}{|>{\centering\arraybackslash}p{10mm}|}{\textbf{概念}} & \multicolumn{1}{|>{\centering\arraybackslash}p{36mm}|}{\textbf{要点}} & \multicolumn{1}{|>{\centering\arraybackslash}p{33mm}|}{\textbf{记号／范围}} \\
\hline
...
\bindp ②操作：先把目标向量沿几何路径分解为基底向量的和，再把各段按比例关系代入基底。

\bindp ③收束：比较等式两边系数求出待定值，防漏写中间段。
...
\bindp ②操作：核对充要条件的前提（两向量不共线或三点不共线）是否齐备，再对缺前提的说法举共线反例排除。

\bindp ③收束：最后汇总正确序号定选项。
...
\bindp ②操作：按定义 a·b=|a||b|cosθ 逐式展开核验，展开后移项验算。

\bindp ③收束：警惕向量无除法、数量积无结合律等误用。

\tjdnr{六｜数量积求夹角与投影（非坐标法）}{例1}{中档（知识点三）}{}

\noindent\begin{minipage}[t]{0.56\linewidth}如图，在正方体\(ABCD - A_{1}B_{1}C_{1}D_{1}\)中，求向量\(\overrightarrow{BC_{1}}\)与\(\overrightarrow{AC}\)的夹角的大小．\end{minipage}\hfill\begin{minipage}[t]{0.40\linewidth}\centering\raisebox{\dimexpr-\height+\ht\strutbox\relax}{\includegraphics[width=\linewidth,alt={@@@d8ffc038-cfd6-4d16-b846-f3d81f29b325}]{media/media/image3.png}}\end{minipage}\par
...
\bindp ②操作：先平移向量使起点共点或选不共面的三个向量作基底，再把相关向量用基底表示并算出数量积与模长。

\bindp ③收束：最后代入夹角公式并按向量夹角范围 [0,π] 取角。
...
\bindp ②操作：第一步用数量积把条件翻译成不等式或方程（锐角 cos>0、钝角 cos<0、垂直 cos=0）。

\bindp ③收束：再求解并剔除使两向量共线的参数，最后写出参数值或取值范围。

\tjdnr{八｜数量积求距离（展开法）}{例1}{中档（知识点三）}{}

\noindent\begin{minipage}[t]{0.56\linewidth}如图，在大小为45°的二面角A\-EF\-D中，四边形ABFE，CDEF都是边长为1的正方形，则B，D两点间的距离是()\end{minipage}\hfill\begin{minipage}[t]{0.40\linewidth}\centering\raisebox{\dimexpr-\height+\ht\strutbox\relax}{\includegraphics[width=\linewidth,alt={@@@494a628b97d74c2894c135f69e8f1eb7}]{media/media/image4.png}}\end{minipage}\par

\bindopt \makebox[\dimexpr0.5\linewidth-1em\relax][l]{A．\(\sqrt{3}\)；}\makebox[\dimexpr0.5\linewidth-1em\relax][l]{B．\(\sqrt{2}\)；}

\bindopt \makebox[\dimexpr0.5\linewidth-1em\relax][l]{C．1；}\makebox[\dimexpr0.5\linewidth-1em\relax][l]{D．\(\sqrt{3 - \sqrt{2}}\)}
...
\bindp ②操作：第一步把待求线段对应的向量写成已知棱向量的首尾相接和链，再对模长平方作数量积展开，逐项代入模长与夹角（端点处两条垂线段的夹角由二面角决定，注意取 45° 还是 135°）。

\bindp ③收束：最后开方得距离。

\tjdnr{九｜数量积求距离（折叠矩形）}{例1}{中档（知识点三）}{已知矩形\(ABCD\)中，\(AB = 1\)，\(BC = \sqrt{3}\)，将矩形\(ABCD\)沿对角线\(AC\)折起，使平面\(ABC\)与平面\(ACD\)垂直，则\(|\overrightarrow{BD}| =\)（~~~~）．}

\bindopt \makebox[\dimexpr0.25\linewidth-0.5em\relax][l]{A．\(\frac{\sqrt{10}}{2}\)；}\makebox[\dimexpr0.25\linewidth-0.5em\relax][l]{B．\(\frac{\sqrt{6}}{2}\)；}\makebox[\dimexpr0.25\linewidth-0.5em\relax][l]{C．\(\frac{\sqrt{5}}{2}\)；}\makebox[\dimexpr0.25\linewidth-0.5em\relax][l]{D．2}
...
\bindp \penalty10000\noindent\makebox[\linewidth][c]{\includegraphics[width=40mm,alt={@@@e34e5148-bca3-440e-84b1-3cbf33fe2cc1}]{media/media/image5.png}}\par\penalty10000
...
\bindp ②操作：第一步在折后图形中过两点对折痕作垂线定出垂足，再把目标向量写成两段垂线与折痕段的和链并平方展开。

\bindp ③收束：最后利用面面垂直带来的向量垂直逐项化简、开方得距离。

\vbox{

\huaxing{课}{堂}{评}{价}{知识评价\quad 素养形成}

\jiancestem{{\heiti 1．}{\fontsize{8pt}{11pt}\selectfont\color{midgray}〔简单（知识点三）〕}已知 \(|\overrightarrow{a}|=2\)，\(|\overrightarrow{b}|=3\)，\(\langle\overrightarrow{a},\overrightarrow{b}\rangle=60^\circ\)，则 \(\overrightarrow{a}\cdot\overrightarrow{b}=\)\kongbai{}．{\zhuzhu{（提示：\(\overrightarrow{a}\cdot\overrightarrow{b}=|\overrightarrow{a}||\overrightarrow{b}|\cos\langle\overrightarrow{a},\overrightarrow{b}\rangle\)）}}}

\ansline{\ansul{3}}

\jiexi{\(\overrightarrow{a}\cdot\overrightarrow{b}=|\overrightarrow{a}||\overrightarrow{b}|\cos60^\circ=2\times3\times0.5=3\)．}

\jiancestem{{\heiti 2．}{\fontsize{8pt}{11pt}\selectfont\color{midgray}〔简单（知识点二）〕}设 \(\overrightarrow{e_1}\)，\(\overrightarrow{e_2}\) 不共线，\(\overrightarrow{a}=2\overrightarrow{e_1}+\overrightarrow{e_2}\)，\(\overrightarrow{b}=4\overrightarrow{e_1}+k\overrightarrow{e_2}\)，若 \(\overrightarrow{a}\parallel\overrightarrow{b}\)，则 \(k=\)（~~~~）}

\bindopt \makebox[\dimexpr0.25\linewidth-0.5em\relax][l]{A．\(\frac{1}{2}\)；}\makebox[\dimexpr0.25\linewidth-0.5em\relax][l]{B．\(2\)；}\makebox[\dimexpr0.25\linewidth-0.5em\relax][l]{C．\(-\frac{1}{2}\)；}\makebox[\dimexpr0.25\linewidth-0.5em\relax][l]{D．\(3\)}

\ansline{\ansul{B}}

\jiexi{\(\overrightarrow{b}=\lambda\overrightarrow{a}\Rightarrow4=2\lambda\Rightarrow\lambda=2\)，\(k=2\)，选 B．}

\jiancestem{{\heiti 3．}{\fontsize{8pt}{11pt}\selectfont\color{midgray}〔简单（知识点三）〕}设 \(\overrightarrow{a}\)，\(\overrightarrow{b}\) 均为非零空间向量，则“\(\overrightarrow{a}\cdot\overrightarrow{b}=0\)”是“\(\overrightarrow{a}\perp\overrightarrow{b}\)”的（~~~~）}
...
\jiancestem{{\heiti 4．}{\fontsize{8pt}{11pt}\selectfont\color{midgray}〔简单（知识点二）〕}在空间四边形 \(ABCD\) 中，\(E\)，\(F\) 分别为 \(AB\)，\(CD\) 的中点，则 \(\frac{1}{2}(\overrightarrow{AD}+\overrightarrow{BC})=\)（~~~~）}

\bindopt \makebox[\dimexpr0.25\linewidth-0.5em\relax][l]{A．\(2\overrightarrow{EF}\)；}\makebox[\dimexpr0.25\linewidth-0.5em\relax][l]{B．\(-\overrightarrow{EF}\)；}\makebox[\dimexpr0.25\linewidth-0.5em\relax][l]{C．\(\overrightarrow{FE}\)；}\makebox[\dimexpr0.25\linewidth-0.5em\relax][l]{D．\(\overrightarrow{EF}\)}
