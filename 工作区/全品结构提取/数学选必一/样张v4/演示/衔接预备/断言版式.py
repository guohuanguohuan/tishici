# -*- coding: utf-8 -*-
r"""版式断言（演示波④衔接预备）：承 演示/章末总结/断言版式.py 拷贝改造，差异逐条登记：
  · 路径改 演示/衔接预备；版式同 A4 竖放双栏（PAGE_W=595.276/PAGE_H=841.89、四边 15mm、
    栏缝 7.5mm、COLW≈244.49pt）＋页面尺寸断言；
  · 花形行顶 ≤60mm 硬门改锚「衔接预备」花形行文字顶缘（本件花形行在课时标题之后，
    版心顶起实测登记）；首题号改「衔接1」顶缘实测登记；
  · 灰档全集 ⊆ {119, 221}（777777 注记档＋DDDDDD 页脚块，纯黑白豁免）口径同章末总结；
  · 栏利用率不设硬门：本件单页（演示件不足页按参差收尾登记，任务书豁免），逐栏实测登记；
  · 件名在场断言：p1「1.2.1 空间中的点、直线与空间向量＋衔接预备」；
...
for blk in p1.get_text('dict')['blocks']:
    for ln in blk.get('lines', []):
        ...
        if hua_top is None and re.match(r'^衔接预备$', txt):
            hua_top = ln['bbox'][1]
        if xt_top is None and re.match(r'^衔接1', txt):
            xt_top = ln['bbox'][1]
h_mm = (hua_top - BODY_TOP) / PT
check('花形行顶 ≤60mm（版心顶→花形行「衔接预备」顶缘）', h_mm <= 60, f'{h_mm:.1f}mm')
xt_mm = (xt_top - BODY_TOP) / PT
print(f'       （登记）首题号（衔接1）顶缘：版心顶起 {xt_mm:.1f}mm')
...
check('件名在场（p1）：1.2.1 课时名＋衔接预备', '1.2.1空间中的点、直线与空间向量' in t1 and '衔接预备' in t1)
