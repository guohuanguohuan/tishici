# 收尾B e2e 复跑（2026-09-06，生产工具＋终态全件PDF硬链接；零COM）

== A) gen-mapping（定族更正验证） exit=0 ==
映射表已生成: 10 行
...
== make-p1 exit=0 ==
== B) --run --mapping 锚点映射表v3.json（十件终态PDF） exit=0 ==
Consider using the pymupdf_layout package for a greatly improved page layout analysis.
断言① PASS 10/10；阴性对照: ('FAIL', '首页正文区未见锚点块（块数=3 正文区行数=1）')
B) 断言① PASS 10/10；阴性对照=FAIL(期望FAIL, ok=True)
== C) 诊断 --run（docx自产表，定族更正后） exit=0 ==
C) 断言① FAIL=['89']（预期：题名内嵌公式件〔89等〕docx自产锚点含文字层不可见串→假阴性；证注册表--mapping为权威源）

== 汇总 ==
SUMMARY_E2E mapping_v3: ①10/10=True 阴性对照FAIL=True → PASS
