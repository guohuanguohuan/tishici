...
输入：子步7盖章记录（件级行start/N同源）＋节页码_子步8.json（二级节级行定位值）。
...
S7 = os.path.join(os.path.dirname(HERE), '子步7')
BASE = r'C:\提示词\高中数学\高中数学同步'
TARGET = os.path.join(BASE, '人教B版选必1·册目录页.docx')

# ---- 输入1：盖章记录 ----
rec = {}
for ln in open(os.path.join(S7, '盖章记录_子步7.md'), encoding='utf-8'):
...
# ---- 输入2：节页码（二级节，按章归并） ----
loc = json.load(open(os.path.join(S7, '节页码_子步8.json'), encoding='utf-8'))
...
shutil.copy2(TARGET, os.path.join(HERE, 'bak', '人教B版选必1·册目录页.docx'))
...
json.dump({'rows': len(rows), 'p_rows': ok,
          ...
          open(os.path.join(HERE, 'build_子步8.json'), 'w', encoding='utf-8'), ensure_ascii=False, indent=1)
print('build_子步8.json 落盘')
