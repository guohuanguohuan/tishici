# -*- coding: utf-8 -*-
r"""门-值快照键型判模.py — M3 S2 课时16·值快照逐字零漂＋全值型＋括线判模断言（波1臂5）。

断言面（母版同构改造）：
  ①值快照（字节级）：main.tex 各键 \ansitem 第二参 ≡ 答案侧「值：」行原文——课时16
    全 50 键均值型逐字全等零换算（本片无过程型/锚制键，PROC/锚制集撤除）。
  ②键型：50 键全「值」，与键序逐位对齐。
  ③括线判模（双证）：静态＝块内容估高（wlen：全角 1、ASCII 0.5，剔 \命令；栏宽 23 字
    口径）est＝ceil，est＞8 → 括线模；断言 tex 括线集 ≡ {est＞8 集}（29 键，边界干净：
    灰底 max est=8／括线 min est=9）；渲染面＝PDF 绘图对象硬计数：
    ansbg 灰底大矩形 21（＝50−29）、ansrule 长线 60（括线 29 块×2＋尾框 2）、
    尾框线在末页恰 2。逐键估高读数落盘 值台账底稿.json。
...
SIDE = 'C:/提示词/工作区/M3-第2章量产0913/成卷/题面库/课时16-2.7.2抛物线性质-答案侧.md'
...
check('括线块数=29 且 灰底块数=21', len(ruled_set) == 29 and len(items) - len(ruled_set) == 21,
      f'括线{len(ruled_set)} 灰底{len(items) - len(ruled_set)}')

# PDF 渲染面：灰底大矩形（ansbg F0F0F0≈0.941）＝21；ansrule 长线（7A7A7A≈0.478）
# ＝括线 29 块×上下 2 线＋尾块框 2 线＝60。课时16 末页与括线块共存，尾框判定改为
# 文本锚定：以「笔记与错题整理」所在栏中紧邻其上/下的两条 ansrule 长线为尾框对。
...
doc.close()
check('渲染面：灰底大矩形＝21', gray_n == 21, f'{gray_n}')
check('渲染面：ansrule 长线＝29块×2＋尾框2＝60', rule_lines == 60, f'{rule_lines}')
check('渲染面：尾块框线（文本锚定上下各一线）', tail_ok, tail_detail)
check('详解覆盖 50/50', all(k in note_vals for k in keyseq), f'{len(note_vals)}')
for it in items:
