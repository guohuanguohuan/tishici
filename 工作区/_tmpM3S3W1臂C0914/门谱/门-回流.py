# -*- coding: utf-8 -*-
r"""门-回流.py — M3 S3 W1臂C 课时06B 练习件·回流门（零强制跳页）＋件型纪律断言。
照抄 母版门谱同构，换 PIECE/MANIFEST；件型纪律按本片实配：
  \duoxuan＝1（manifest 期望值多选数，T10）；\liubai＝2（T15 两问 24mm／T16 单问 16mm，
  装配序读数）；\kongda＝4（T11~T14 填空题面空位印答）。
...
MANIFEST = 'C:/提示词/工作区/M3-第2章量产0913/成卷/题面库/manifest/课时06B.manifest.json'
...
check('\\liubai 计数＝2（解答题 T15/T16，头标槽型实录）', n_liubai == 2, f'{n_liubai}')
check('\\liubai 装配序高度＝[24mm,16mm]（T15 两问档／T16 无小问默认档；16~40mm 带内）',
      liubai_h == ['24mm', '16mm'], f'{liubai_h}')

n_kongda = len(re.findall(r'\\kongda(?![a-zA-Z])', body))
check('\\kongda 计数＝4（T11~T14 填空题面空位印答）', n_kongda == 4, f'{n_kongda}')
