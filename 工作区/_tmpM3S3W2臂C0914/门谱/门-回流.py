...
check(f'\\liubai 计数＝解答题数 10（以头标槽型实录为准；练键共{n_jieda}）', n_liubai == 10, f'{n_liubai}')
check('\\liubai 逐题高度＝32/16/16/16/24/16/24/16/24/24mm（题1 四小问32、题8/10/15/16 两问24、余无小问16；16~40mm 带内）',
      liubai_h == ['32mm', '16mm', '16mm', '16mm', '24mm', '16mm', '24mm', '16mm', '24mm', '24mm'], f'{liubai_h}')

n_kongda = len(re.findall(r'\\kongda(?![a-zA-Z])', body))
check('\\kongda 计数＝2（题5/题13 填空印答位）', n_kongda == 2, f'{n_kongda}')
