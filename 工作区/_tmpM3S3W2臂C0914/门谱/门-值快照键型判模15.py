# -*- coding: utf-8 -*-
r"""门-值快照键型判模15.py — M3 S3 课时15 练习件（W2 臂C）·值快照逐字零漂＋键型＋全线括线判模断言。
照 门-值快照键型判模.py（课时14 版）同制：改 PIECE/SIDE/MANIFEST 指向课时15；
本片无过程型键，PROC 置空集（全 16 键走逐字全等档）；底稿落盘改 值台账底稿15.json（不覆课时14 件）。

用法: python 门-值快照键型判模15.py
退出码: 0＝全过；1＝有红。红线：件树/题面库只读；唯一写件＝值台账底稿15.json（本过程件目录）。
"""
import fitz  # pymupdf
import io
import json
...
OUT = os.path.join(os.path.dirname(os.path.abspath(__file__)), '值台账底稿15.json')
