# -*- coding: utf-8 -*-
r"""门-回流.py — M3 S3 W1臂A 课时02 练习件·回流门（零强制跳页）＋件型纪律断言。
（照抄母版门谱 工作区/_tmpM3S3母版0914/门谱/门-回流.py，PIECE/MANIFEST 常量换片；
逐片期望值：\duoxuan＝manifest 多选数 1（E6）；\liubai＝解答题数 4（E13/E14/E15＝16mm
无小问默认档、E16 四小问＝32mm）；\kongda＝3（E10/E11/E12 填空印答位）。）
...
断言面：main.tex 剔注释后：
  ...
  ②练习件件型纪律：\begin{ketang} ×0；\tailfill 恰 1（置 \end{multicols} 前）；
    \duoxuan 计数＝manifest 期望值多选数（本片 1，≤4 合规）；
    \liubai 计数＝片内解答题数 4 且逐题高度 16~40mm 带内（16mm×3＋32mm×1）；
    \kongda 计数＝3（填空印答位，两档等形开关宏）。
用法: python 门-回流-课时02.py     退出码: 0＝全过；1＝有红。
"""
import io
import json
import os
...
check('\\liubai 计数＝解答题数 4（E13~E16，题面侧头标实录）', n_liubai == 4, f'{n_liubai}')
inband = all(16.0 <= float(h[:-2]) <= 40.0 for h in liubai_h)
check('\\liubai 逐题高度 16~40mm 带内（16mm×3 无小问默认档＋32mm×1 四小问）',
      inband and liubai_h == ['16mm', '16mm', '16mm', '32mm'], f'{liubai_h}')

n_kongda = len(re.findall(r'\\kongda(?![a-zA-Z])', body))
check('\\kongda 计数＝3（E10/E11/E12 填空印答位）', n_kongda == 3, f'{n_kongda}')
