# -*- coding: utf-8 -*-
r"""门-值快照键型判模.py — M3 S3 W1臂A 课时02 练习件·值快照逐字零漂＋键型两档＋全线括线判模断言。
（照抄母版门谱 工作区/_tmpM3S3母版0914/门谱/门-值快照键型判模.py，PIECE/SIDE/MANIFEST/OUT/PROC
常量换片：本片 16 键答案侧值全为具体值，PROC（「证明见详解．」过程型）＝空集。）

断言面：
  ①值快照（字节级）：main.tex 各键 \ansitem 第二参 ≡ 答案侧「值：」行原文（16 键全逐字全等档）。
  ②键型两档：值型 16＋过程型 0＝16，与 manifest 练键序逐位对齐；印面号 1..16 连号。
  ...
  逐键估高读数落盘 底稿-课时02.json（>8行→括线模口径：全线括线下自然满足，读数备查）。
用法: python 门-值快照键型判模-课时02.py   退出码: 0＝全过；1＝有红。
红线：件树/题面库只读；唯一写件＝底稿-课时02.json（本过程件目录）。
"""
import fitz  # pymupdf
import io
import json
...
OUT = os.path.join(os.path.dirname(os.path.abspath(__file__)), '底稿-课时02.json')
