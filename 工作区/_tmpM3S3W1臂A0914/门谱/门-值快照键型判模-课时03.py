# -*- coding: utf-8 -*-
r"""门-值快照键型判模.py — M3 S3 W1臂A 课时03 练习件·值快照逐字零漂＋键型两档＋全线括线判模断言。
（照抄母版门谱 工作区/_tmpM3S3母版0914/门谱/门-值快照键型判模.py，PIECE/SIDE/MANIFEST/OUT/PROC
常量换片：本片过程型键仅 E11（值＝「证明见详解．」逐字）；E5 值「真命题．证明见详解．」、
E8 值「一定．理由见详解．」含过程语但系答案侧原文，走值型逐字全等档。）

断言面：
  ①值快照（字节级）：main.tex 各键 \ansitem 第二参 ≡ 答案侧「值：」行原文
      （值型 15 键逐字全等＋过程型 1 键全等）。
  ②键型两档：值型 15＋过程型 1＝16，与 manifest 练键序逐位对齐；印面号 1..16 连号。
  ③全线括线判模（S3 波次方案 §四.3）：导言 \ansblockgrayfalse 恰 1 且在 \begin{document} 前；
      件内零逐块 {\ansblockgrayfalse 包装；渲染面硬计数：ansbg 灰底大矩形＝0、
      ansrule 长线＝16 块×上下 2 线＋尾框 2＝34、尾框对＝末页绘图序最后两根（同列纵列）。
  逐键估高读数落盘 底稿-课时03.json（>8行→括线模口径：全线括线下自然满足，读数备查）。
用法: python 门-值快照键型判模-课时03.py   退出码: 0＝全过；1＝有红。
红线：件树/题面库只读；唯一写件＝底稿-课时03.json（本过程件目录）。
"""
import fitz  # pymupdf
import io
import json
...
SIDE = 'C:/提示词/工作区/M3-第2章量产0913/成卷/题面库/课时03-方向向量与法向量-答案侧.md'
MANIFEST = 'C:/提示词/工作区/M3-第2章量产0913/成卷/题面库/manifest/课时03.manifest.json'
OUT = os.path.join(os.path.dirname(os.path.abspath(__file__)), '底稿-课时03.json')

PROC = {'2章-练-课时03-E11'}  # 过程型仅 E11；E5/E8 值含过程语系答案侧原文，逐字照录
