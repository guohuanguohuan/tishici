# -*- coding: utf-8 -*-
r"""门-守恒对号.py — M3 S2 波1 臂4·课时09 圆与圆的位置关系·答案块守恒断言（双档）＋对号门＋尾块门＋页数门。
适配自母版门谱（_tmpM3S2母版0914/门谱）：PIECE/MANIFEST/键数 23→21。
用法: python 门-守恒对号.py   退出码: 0＝全过；1＝有红。
"""
import io
import json
import os
...
check(f'ansitem 号序≡1..{NKEY} 连号', nums == list(range(1, NKEY + 1)), f'{nums if len(nums) != NKEY else "21枚"}')
note_cnt = len(RE_NOTE.findall(src))
check('ansnote{详解} 计数>0 且=21 键均有详解行', note_cnt == NKEY, f'{note_cnt}')
...
check('true [答案] 总计=21', pdf['true']['text'].count('[答案]') == NKEY,
