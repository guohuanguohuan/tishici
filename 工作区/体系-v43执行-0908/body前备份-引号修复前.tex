\vspace{1.7mm}%  v4.3 花形前距（0908 实测回填）：课前预习花形为 multicols 栏首元素，行首胶被 TeX 弃置，前距须在栏区外给——设计 4.5 实出墨距 2.82，+1.7 → 4.5±0.8 窗
\begin{multicols}{2}
...
\vspace{-3.8mm}
\noindent\begin{tabular}{!{\vline width 0.8pt}>{\centering\arraybackslash}p{16mm}!{\vline width 0.4pt}>{\raggedright\arraybackslash}p{35mm}!{\vline width 0.4pt}>{\raggedright\arraybackslash}p{29mm}!{\vline width 0.8pt}}
\thickhline
...
\vspace{-3.8mm}
\noindent\begin{tabular}{!{\vline width 0.8pt}>{\centering\arraybackslash}p{10mm}!{\vline width 0.4pt}>{\raggedright\arraybackslash}p{37mm}!{\vline width 0.4pt}>{\raggedright\arraybackslash}p{32mm}!{\vline width 0.8pt}}
\thickhline
...
\tiaomu{2}{数量积：定义 \(\overrightarrow{a}\cdot\overrightarrow{b}=|\overrightarrow{a}||\overrightarrow{b}|\cos\langle\overrightarrow{a},\overrightarrow{b}\rangle\)；规定\kongda{零向量}与任意向量的数量积为 0．性质：\(\overrightarrow{a}\perp\overrightarrow{b}\Leftrightarrow\overrightarrow{a}\cdot\overrightarrow{b}=0\)；\(\overrightarrow{a}\cdot\overrightarrow{a}={{\overrightarrow{a}}^{2}}={\left| \overrightarrow{a} \right|}^{2}\)；\(\left| \overrightarrow{a}\cdot\overrightarrow{b} \right|\leq\left| \overrightarrow{a} \right|\left| \overrightarrow{b} \right|\)．\zhuzhu{【微提醒】零向量与任意向量的数量积为0．}\zhuzhu{求空间角与距离的核心运算（支撑条目16、17、34）；注意数量积不满足结合律，零向量与任意向量的数量积为0。}}
...
\bindp \par\vspace{1.9mm}\penalty10000\noindent\makebox[\linewidth][c]{\includegraphics[width=60mm,alt={@@@5da5e1ea-9d33-4579-a56f-804b4a72c894}]{media/media/sub3_B_4.png}}\par\vspace{-1.7mm}\penalty10000
...
{\setlength{\tabcolsep}{3pt}\arrayrulecolor{gray122}
\vspace{-3.8mm}
\noindent\begin{tabular}{!{\vline width 0.8pt}>{\centering\arraybackslash}p{10mm}!{\vline width 0.4pt}>{\raggedright\arraybackslash}p{36mm}!{\vline width 0.4pt}>{\raggedright\arraybackslash}p{33mm}!{\vline width 0.8pt}}
\thickhline
...
\zhenti{（2）}{两个非零空间向量的数量积大于 \(0\)，则这两个向量的夹角为锐角．}{×}{数量积大于 0 仅保证 \(\cos\theta>0\)，夹角 \(\theta\in[0^\circ,90^\circ)\)；\(\theta=0^\circ\)（同向）时数量积也为正而非锐角，故×．}

\huaxing{课}{中}{探}{究}{考点探究\quad 素养小结}

\tjdnr{一｜空间向量的概念辨析}{例1}{简单（知识点一）}{下列说法正确的是（~~~~）}

\bindopt {\fontsize{10.5pt}{19pt}\selectfont A．若\(\left| \overrightarrow{a} \right| = \allowbreak\left| \overrightarrow{b} \right|\)，则\(\overrightarrow{a} = \overrightarrow{b}\)或\(\overrightarrow{a} = - \overrightarrow{b}\)；\par}
...
\jiexi{A 规定成立；B 模为 0 即零向量；C 相等必等模；D 平行不需等模（反例 \(\overrightarrow{a}=2\overrightarrow{b}\)），故选 D．}
...
\tjdnr{二｜空间向量的线性运算}{例1}{简单（知识点二）}{}

\noindent\begin{minipage}[t]{0.56\linewidth}\raggedright 在直三棱柱\(ABC - A_{1}B_{1}C_{1}\)中，若\(\overrightarrow{CA} = \overrightarrow{a},\overrightarrow{CB} = \overrightarrow{b},\overrightarrow{CC_{1}} = \overrightarrow{c},\)，则\(\overrightarrow{A_{1}B}\)=\kongbai{}.(用\(\overrightarrow{a},\overrightarrow{b},\overrightarrow{c}\)表示)\end{minipage}\hfill\begin{minipage}[t]{0.40\linewidth}\centering\raisebox{\dimexpr-\height+\ht\strutbox\relax}{\includegraphics[width=\linewidth,alt={@@@4e3aa752498e415b9cc3d6d60f7bb3fe}]{media/media/image1.png}}\end{minipage}\par
...
\tjdnr{三｜用基底表示向量}{例1}{简单（知识点二）}{}

\noindent\begin{minipage}[t]{0.56\linewidth}\raggedright 已知正方体\emph{ABCD}－\emph{A\textsubscript{1}B\textsubscript{1}C\textsubscript{1}D\textsubscript{1}}中，\(\overrightarrow{A_{1}E}\)＝\(\frac{1}{4}\) \(\overrightarrow{A_{1}C_{1}}\)，若\(\overrightarrow{AE} =\)\emph{x}\(\overrightarrow{AA_{1}}\)＋\emph{y}(\(\overrightarrow{AB}\)＋\(\overrightarrow{AD}\))，则\emph{x}＝\kongbai{}，\emph{y}＝\kongbai{}.\end{minipage}\hfill\begin{minipage}[t]{0.40\linewidth}\centering\raisebox{\dimexpr-\height+\ht\strutbox\relax}{\includegraphics[width=\linewidth,alt={@@@b0b55c9f-4634-4bba-b096-bc6dd0cf7747}]{media/media/image2.png}}\end{minipage}\par
...
\xiaojie{①识别：题干给出 p=xa+yb 或 MP=xMA+yMB 一类线性表示与“共面”结论的互推说法判断时，判归本题型。}
...
\bindopt {\fontsize{10.5pt}{19pt}\selectfont A．\({\overrightarrow{a}}^{2} = \allowbreak\left| \overrightarrow{a} \right|^{2}\)；\par}
...
\bindp {\kaishu ②操作：按定义 a·b=|a||b|cosθ 逐式展开核验，展开后移项验算。}

\bindp {\kaishu ③收束：警惕向量无除法、数量积无结合律等误用。}

\tjdnr{六｜数量积求夹角与投影（非坐标法）}{例1}{中档（知识点三）}{}

\noindent\begin{minipage}[t]{0.56\linewidth}\raggedright 如图，在正方体\(ABCD - A_{1}B_{1}C_{1}D_{1}\)中，求向量\(\overrightarrow{BC_{1}}\)与\(\overrightarrow{AC}\)的夹角的大小．\end{minipage}\hfill\begin{minipage}[t]{0.40\linewidth}\centering\raisebox{\dimexpr-\height+\ht\strutbox\relax}{\includegraphics[width=\linewidth,alt={@@@d8ffc038-cfd6-4d16-b846-f3d81f29b325}]{media/media/image3.png}}\end{minipage}\par
...
\bindp {\kaishu ②操作：第一步用数量积把条件翻译成不等式或方程（锐角 cos>0、钝角 cos<0、垂直 cos=0）。}

\bindp {\kaishu ③收束：再求解并剔除使两向量共线的参数，最后写出参数值或取值范围。}

\tjdnr{八｜数量积求距离（展开法）}{例1}{中档（知识点三）}{}

\noindent\begin{minipage}[t]{0.56\linewidth}\raggedright 如图，在大小为45°的二面角A\-EF\-D中，四边形ABFE，CDEF都是边长为1的正方形，则B，D两点间的距离是()\end{minipage}\hfill\begin{minipage}[t]{0.40\linewidth}\centering\raisebox{\dimexpr-\height+\ht\strutbox\relax}{\includegraphics[width=\linewidth,alt={@@@494a628b97d74c2894c135f69e8f1eb7}]{media/media/image4.png}}\end{minipage}\par
...
\bindp {\kaishu ②操作：第一步把待求线段对应的向量写成已知棱向量的首尾相接和链，再对模长平方作数量积展开，逐项代入模长与夹角（端点处两条垂线段的夹角由二面角决定，注意取 45° 还是 135°）。}

\bindp {\kaishu ③收束：最后开方得距离。}

\tjdnr{九｜数量积求距离（折叠矩形）}{例1}{中档（知识点三）}{已知矩形\(ABCD\)中，\(AB = 1\)，\(BC = \sqrt{3}\)，将矩形\(ABCD\)沿对角线\(AC\)折起，使平面\(ABC\)与平面\(ACD\)垂直，则\(|\overrightarrow{BD}| =\)（~~~~）．}
...
\bindp \par\vspace{1.9mm}\penalty10000\noindent\makebox[\linewidth][c]{\includegraphics[width=34mm,alt={@@@e34e5148-bca3-440e-84b1-3cbf33fe2cc1}]{media/media/image5.png}}\par\vspace{-1.7mm}\penalty10000
...
\jiexi{\(\overrightarrow{b}=\lambda\overrightarrow{a}\Rightarrow4=2\lambda\Rightarrow\lambda=2\)，\(k=2\)，选 B．}

\jiancestem{{\fontsize{11.4pt}{14pt}\selectfont\heihao 3．}{\fontsize{8pt}{11pt}\selectfont\color{midgray}〔简单（知识点三）〕}\hspace{1.3mm}设 \(\overrightarrow{a}\)，\(\overrightarrow{b}\) 均为非零空间向量，则“\(\overrightarrow{a}\cdot\overrightarrow{b}=0\)”是“\(\overrightarrow{a}\perp\overrightarrow{b}\)”的（~~~~）}
...
\jiexi{非零向量 \(\overrightarrow{a}\cdot\overrightarrow{b}=0\Leftrightarrow\overrightarrow{a}\perp\overrightarrow{b}\)，充要条件，选 C．}

\jiancestem{{\fontsize{11.4pt}{14pt}\selectfont\heihao 4．}{\fontsize{8pt}{11pt}\selectfont\color{midgray}〔简单（知识点二）〕}\hspace{1.3mm}在空间四边形 \(ABCD\) 中，\(E\)，\(F\) 分别为 \(AB\)，\(CD\) 的中点，则 \(\frac{1}{2}(\overrightarrow{AD}+\overrightarrow{BC})=\)（~~~~）}
