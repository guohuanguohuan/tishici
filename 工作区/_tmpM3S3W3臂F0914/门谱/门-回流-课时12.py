# -*- coding: utf-8 -*-
r"""门-回流-课时12.py — M3 S3 W3臂F 课时12 练习件·回流门（零强制跳页）＋件型纪律断言。
（照抄母版门谱 工作区/_tmpM3S3母版0914/门谱/门-回流.py，PIECE/MANIFEST 常量换片；
逐片期望值：\duoxuan＝manifest 多选数 1（难2）；\liubai＝解答题数 12（简1~3/简6~9/中1~中4/难1，
无小问 16mm×10＋两问 24mm×2＝简6/中4）；\kongda＝0（本片填空 0，解答槽题面尾空位一律 \kongbai）。）
用法: python 门-回流-课时12.py     退出码: 0＝全过；1＝有红。
"""
import io
import json
import os
...
MANIFEST = 'C:/提示词/工作区/M3-第2章量产0913/成卷/题面库/manifest/课时12.manifest.json'
...
check('\\liubai 计数＝解答题数 12（题面侧头标实录：简1~3/简6~9/中1~中4/难1）', n_liubai == 12, f'{n_liubai}')
inband = all(16.0 <= float(h[:-2]) <= 40.0 for h in liubai_h)
check('\\liubai 逐题高度 16~40mm 带内（16mm×10 无小问默认档＋24mm×2 两问＝简6/中4）',
      inband and liubai_h == ['16mm', '16mm', '16mm', '24mm', '16mm', '16mm', '16mm',
                              '16mm', '16mm', '16mm', '24mm', '16mm'], f'{liubai_h}')

n_kongda = len(re.findall(r'\\kongda(?![a-zA-Z])', body))
check('\\kongda 计数＝0（本片填空 0；解答槽题面尾空位 \\kongbai 两档等形）', n_kongda == 0, f'{n_kongda}')
