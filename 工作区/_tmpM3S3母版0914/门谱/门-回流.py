...
check(f'\\liubai 计数＝解答题数 {n_jieda}？（以头标槽型实录 7 为准）', n_liubai == 7, f'{n_liubai}')
check('\\liubai 逐题高度＝16mm（本件全题无小问默认档；16~40mm 带内）',
      liubai_h == ['16mm'] * 7, f'{liubai_h}')

n_kongda = len(re.findall(r'\\kongda(?![a-zA-Z])', body))
check('\\kongda 计数＝1（E7 填空印答位）', n_kongda == 1, f'{n_kongda}')
