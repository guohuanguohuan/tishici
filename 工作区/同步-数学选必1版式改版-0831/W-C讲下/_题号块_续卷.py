# -*- coding: utf-8 -*-
# 一次性适配副本（W-C讲下）：A1起点断言参数化QSTART=62（续卷题号62..140）；其余与 工具/题号块三段式.py 逐行一致
# -*- coding: utf-8 -*-
...
def migrate(path, regmd, linkage=False):
    ...
    QSTART = int(os.environ.get('QSTART', '1'))
    assert not gaps and (not seq or seq[0] in (1, QSTART)), '题号序列门控失败: 起=%s 断点=%s' % (
    ...
    L.append('')
    L.append('题号块 %d（门控认定，序列%s..%d连续A1过〔续卷版QSTART=%s〕）｜文本改写 %d（授权）｜纯缩底纹 %d｜退化缩底纹 %d｜'
             '幂等跳过 %d｜重建run %d｜多run归并 %d｜补底纹 %d｜补加粗 %d｜A2形态断言 %d 段全过｜'
             'extract交叉核对 %s'
             % (n_q, ('%d' % seq[0]) if seq else '1', seq[-1] if seq else 0, QSTART, c_rew, c_narrow, c_degr, c_skip, c_runs, c_merged,
                c_shd, c_bold, n_q,
    ...
    print('题号块 %d（%s..%d）｜改写 %d｜缩底纹 %d｜退化 %d｜幂等跳过 %d｜重建run %d｜补底纹 %d｜补加粗 %d｜'
          'A2断言 %d段全过｜交叉核对 %s'
          % (n_q, ('%d' % seq[0]) if seq else '1', seq[-1] if seq else 0, c_rew, c_narrow, c_degr, c_skip, c_runs, c_shd, c_bold,
