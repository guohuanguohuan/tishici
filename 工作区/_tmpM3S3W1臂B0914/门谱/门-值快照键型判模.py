# -*- coding: utf-8 -*-
r"""门-值快照键型判模.py — M3 S3 W1 臂B 练习件（课时04＋05）·值快照逐字零漂＋键型两档＋全线括线判模断言。
承母版 工作区/_tmpM3S3母版0914/门谱/门-值快照键型判模.py 复刻：PIECE/SIDE/MANIFEST/OUT 参数化；
PROC（过程型键集）两片均空（16 键答案侧值全非「证明见详解．」，逐键判值型）；
全局 ansrule 长线钉数 34＝16块×2＋尾框2 两片同构。
用法: python 门-值快照键型判模.py 课时04|课时05
...
if tag_sel not in PIECES:
    print('用法: python 门-值快照键型判模.py 课时04|课时05'); sys.exit(2)
PIECE, SIDE, MANIFEST = PIECES[tag_sel]
OUT = os.path.join(HERE, f'值台账底稿-{tag_sel}.json')
PROC = set()
...
print(f'== {tag_sel} ③ 全线括线判模（导言置定＋零逐块包装＋PDF 渲染面） ==')
