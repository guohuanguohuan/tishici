# -*- coding: utf-8 -*-
r"""门-值快照键型判模-课时16.py — M3 S3 W2臂D 课时16 练习件·值快照逐字零漂＋键型两档＋全线括线判模断言。
（照抄 W2臂B 门谱 门-值快照键型判模-课时08.py，PIECE/SIDE/MANIFEST/OUT/PROC 常量换片：
本片 16 键答案侧值全为具体值（含 16-15 分问值行、16-09 字母＋括注值），PROC（「证明见详解．」过程型）＝空集。）
用法: python 门-值快照键型判模-课时16.py   退出码: 0＝全过；1＝有红。
红线：件树/题面库只读；唯一写件＝底稿-课时16.json（本过程件目录）。
"""
import fitz  # pymupdf
import io
import json
...
OUT = os.path.join(os.path.dirname(os.path.abspath(__file__)), '底稿-课时16.json')
