# -*- coding: utf-8 -*-
r"""门-回流-课时16.py — M3 S3 W2臂D 课时16 练习件·回流门（零强制跳页）＋件型纪律断言。
（照抄 W2臂B 门谱 门-回流-课时08.py，PIECE/MANIFEST 常量换片；
逐片期望值：\duoxuan＝manifest 多选数 1；\liubai＝解答题数 12（题2~8、12~16，
题3 四小题＝32mm、题15 两问＝24mm、余 16mm，按装配序）；\kongda＝0（本片零填空）。）
用法: python 门-回流-课时16.py     退出码: 0＝全过；1＝有红。
"""
import io
import json
import os
...
check('\\liubai 计数＝解答题数 12（题2~8、12~16，题面侧头标实录）', n_liubai == 12, f'{n_liubai}')
inband = all(16.0 <= float(h[:-2]) <= 40.0 for h in liubai_h)
check('\\liubai 逐题高度 16~40mm 带内（题3 四小题 32mm＋题15 两问 24mm＋余 16mm）',
      inband and liubai_h == ['16mm', '32mm', '16mm', '16mm', '16mm', '16mm', '16mm',
                              '16mm', '16mm', '16mm', '24mm', '16mm'], f'{liubai_h}')

n_kongda = len(re.findall(r'\\kongda(?![a-zA-Z])', body))
check('\\kongda 计数＝0（本片零填空）', n_kongda == 0, f'{n_kongda}')
