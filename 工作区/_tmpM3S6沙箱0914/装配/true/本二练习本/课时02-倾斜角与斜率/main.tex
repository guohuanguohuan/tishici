...
% 图债：E1 国旗五星坐标图已于 0914 印前回冲印面（图资源/02-E1.png，源卷①提取；
%   「如图」处 \ansfig 插桩，双档编译三零承证；台账「图债」字段同账销案）。
...
\ansfig{\includegraphics[width=40mm]{figs/02-E1.png}}
...
\tihao{3}\zu{倾斜角与斜率×单选} \tieside{中档(知识点三)}直线\(l\)的倾斜角为\(\alpha\)，且\(\sin\alpha=3/5\)，则直线\(l\)的斜率是\nobreak(\kongwei)
...
\tihao{8}\zu{倾斜角与斜率×单选} \tieside{简单(知识点二)}直线\(m\)过点\(O(0,0)\)，\(A(1,\sqrt{2})\)，其倾斜角为\(\alpha\)，现将直线\(m\)绕原点\(O\)逆时针旋转得到直线\(m'\)：\(y=kx\)，若直线\(m'\)的倾斜角为\(2\alpha\)，则\(k\)的值为\nobreak(\kongwei)
...
\tihao{9}\zu{倾斜角与斜率×单选} \tieside{简单(知识点一)}直线\(2x\cdot\sin 210^\circ-y-2=0\)的倾斜角是\nobreak(\kongwei)
...
\tihao{16}\zu{倾斜角与斜率×解答} \tieside{简单(知识点一)}分别求下列直线的倾斜角：(1) \(y=\sqrt{3}x-2\)；(2) \(x+\sqrt{3}y+1=0\)；(3) \(x=-3\)；(4) \(y=1/2\)．
\liubai[32mm]{解答书写区}
