...
% 知识点预排注记：导学件课时06 落盘后经 S4 印前回核（0914），预排切分与落盘分册确有异动，
%   6 席已按落盘切分改号（题4/6/11/14→知识点一，题9/15→知识点二；题1 抽核候选经键联
%   对读取符，不改）。落盘切分：一＝平行与垂直的判定及求参、二＝位置关系的综合应用、
%   三＝含参直线系与综合问题（照导学件知识导学分册映射）。
% ============================================================
...
\tihao{4}\zu{两直线位置关系的判定×单选} \tieside{中档(知识点一)}已知直线\(l_1\)的倾斜角为\(60^\circ\)，直线\(l_2\)经过点\(A(1,\sqrt{3})\)、\(B(-2,2\sqrt{3})\)，则直线\(l_1\)与\(l_2\)的位置关系是\nobreak(\kongwei)\par
...
\tihao{6}\zu{两直线位置关系的判定×单选} \tieside{中档(知识点一)}若直线\(kx+(1-k)y-3=0\)和直线\((k-1)x+(2k+3)y-2=0\)互相垂直，则\(k=\)\nobreak(\kongwei)
...
\tihao{9}\zu{两直线位置关系的判定×单选} \tieside{中档(知识点二)}过直线\(l_1\)：\(2x+y-3=0\)与\(l_2\)：\(x-3y+2=0\)的交点，并与\(l_1\)垂直的直线的方程为\nobreak(\kongwei)
...
\tihao{11}\zu{两直线位置关系的判定×填空} \tieside{简单(知识点一)}已知三条直线\(l_1\)：\(2x+my+2=0\)（\(m\in R\)），\(l_2\)：\(2x+y-1=0\)，\(l_3\)：\(x+ny+1=0\)（\(n\in R\)），若\(l_1\parallel l_2\)，\(l_1\perp l_3\)，则\(m+n\)的值为\kongda{−1}．
...
\tihao{14}\zu{两直线位置关系的判定×填空} \tieside{中档(知识点一)}已知直线\(l_1\)经过点\(A(3,m)\)，\(B(m-1,2)\)，直线\(l_2\)经过点\(C(1,2)\)，\(D(-2,m+2)\)．
...
\tihao{15}\zu{两直线位置关系的判定×解答} \tieside{中档(知识点二)}已知在\pxparallelogram\(ABCD\)中，\(A(1,2)\)，\(B(5,0)\)，\(C(3,4)\)．
(1) 求点\(D\)的坐标；
