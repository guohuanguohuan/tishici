...
def build_booklet(tag, jianxing_list, outname, jianming_label):
    ...
    guard_stat = {'wrap': 0, 'keep': 0, 'red': 0}
    ...
    for s in srcs:
        ...
        for b in s['blocks']:
            ...
            A('%% ans:%s' % b['key'])
            need = T.guard_need(b['val'])
            guard_stat[need] += 1
            inner = '\\ansitem{%s}{%s}' % (b['seat'], b['val'])
            A(T.GUARD_PREFIX + inner + '}' if need == 'wrap' else inner)
            A('\\ifansbookdetail')
    ...
    bk = reparse_raw(out)
    s_blocks_all = [b for s in srcs for b in s['blocks']]
    g = T.Gates()
    ...
    # 护栏门（器升制①）：全册值行扫描（文本段裸 `_` 无护栏＝0／数学段误包＝0）
    # ＋归一化 diff（册值串↔源值串逐字，护栏外壳除外零字符增删，与钉值门并联）
    ntxt_nomth = 0
    mis = 0
    for b in s_blocks_all:
        txt, mth = T.underscore_ctx(b['val'])
        wrapped = T.guard_need(b['val']) == 'wrap'
        if txt and not wrapped:
            ntxt_nomth += 1
        if mth and wrapped:
            mis += 1
        if txt and mth:
            g.check('护栏：值行两态并存（源件侧转工单）', False, '键 %s' % b['key'])
    g.check('护栏：全册值行文本段裸下划线且无护栏＝0', ntxt_nomth == 0, '违规 %d' % ntxt_nomth)
    g.check('护栏：数学段裸 `_` 误包＝0', mis == 0, '违规 %d' % mis)
    g.check('归一化diff：册值串↔源值串逐字（护栏外壳除外零字符增删）', bk['vals'] == vals,
            '%d/%d 逐字零增删（wrap %d/keep %d，与钉值门并联）'
            % (N, N, guard_stat['wrap'], guard_stat['keep']))
    # 编译＋log/PDF 门
