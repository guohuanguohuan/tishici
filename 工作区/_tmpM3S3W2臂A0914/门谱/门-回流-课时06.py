...
check(f'\\liubai 计数＝解答题数 {n_jieda}？（以头标槽型实录 2 为准）', n_liubai == 2, f'{n_liubai}')
check('\\liubai 逐题高度＝T15/T16 均两问（16~40mm 带内）',
      liubai_h == ['24mm', '24mm'], f'{liubai_h}')

n_kongda = len(re.findall(r'\\kongda(?![a-zA-Z])', body))
check('\\kongda 计数＝6（T11/T12 单空＋T13 两空＋T14 两问共6空印答位）', n_kongda == 6, f'{n_kongda}')
