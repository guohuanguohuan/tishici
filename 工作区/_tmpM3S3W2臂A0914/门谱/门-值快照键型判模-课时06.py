# -*- coding: utf-8 -*-
r"""门-值快照键型判模.py — M3 S3 课时01 练习件母版·值快照逐字零漂＋键型两档＋全线括线判模断言。

断言面：
  ①值快照（字节级）：main.tex 各键 \ansitem 第二参 ≡ 答案侧「值：」行原文——
      值型 12 键逐字全等；过程型 E8/E11/E13/E14（答案侧值＝「证明见详解．」）亦逐字全等。
      本件无 T 键（拓区隔离），故全件 16 键均为逐字全等档、零锚制。
  ②键型两档：值型 12＋过程型 4＝16，与 manifest 练键序逐位对齐；印面号 1..16 连号。
  ③全线括线判模（S3 波次方案 §四.3）：导言 \ansblockgrayfalse 恰 1 且在 \begin{document} 前
      （一次置定、件内不切换）；件内零逐块 {\ansblockgrayfalse 包装；
      渲染面硬计数：ansbg 灰底大矩形＝0（练习件禁灰底）、ansrule 长线＝16 块×上下 2 线＋尾框 2＝34、
      尾框线在末页恰 2。
  逐键估高读数落盘 值台账底稿.json（>8行→括线模口径：全线括线下自然满足，读数备查）。
...
SIDE = 'C:/提示词/工作区/M3-第2章量产0913/成卷/题面库/课时06-两条直线的位置关系-答案侧.md'
MANIFEST = 'C:/提示词/工作区/M3-第2章量产0913/成卷/题面库/manifest/课时06.manifest.json'
OUT = os.path.join(os.path.dirname(os.path.abspath(__file__)), '底稿-课时06.json')

PROC = set()  # 本片 16 键答案侧值全为具体值，无「证明见详解．」过程键
...
# ＝16 块×上下 2 线＋尾块框 2 线＝34；尾块框线恒在末页恰 2。
...
# 末页断言：跨栏括线块规则线可自然落到末页，尾框对＝末页绘图序最后两根
# （同列、下根在上根下方、其后无同类线）；全局 34 根已钉死 32 块线＋2 尾线。
tail_pair = False
doc = fitz.open(os.path.join(PIECE, 'main-true.pdf'))
page = doc[len(doc) - 1]
