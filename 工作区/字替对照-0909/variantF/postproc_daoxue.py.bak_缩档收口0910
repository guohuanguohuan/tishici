...
片E 图文绕排回流（2026-09-10，用户终审 #40）：5 组图文并排改「绕图回流」——图旁文字窄栏（几何档 3c
不动）＋图底以下文字恢复通栏。实现＝文本切分法（SIDE_DEF['split'] 标定切分点）：图旁前缀仍走
「左文字 minipage＋右图 minipage」并排（窄栏断行与修复轮全窄形逐行一致，三窗几何不回退），
...
FIG = {'sub3_B_4.png': ('under', ''),   # 条目3 投影三联图：片G 0910 下置居中满栏（源 86.0mm 超栏封顶 \linewidth；旧 v4.3 居中 60mm）
       ...
       'image3.png': ('under', ''),     # 探究点六例1（题6）题干图：片G 0910 下置居中·84.0mm 封顶（源 86.0mm，同比缩 0.9767）
       'image4.png': ('under', ''),     # 探究点八例1（题9）题干图：片G 0910 下置居中·源尺寸 66.3mm（旧绕图回流）
       'image5.png': ('under', '')}     # 探究点九例1（题10）图：片G 0910 下置居中·54.7mm（素材自然外框 56.564mm，同比缩 0.9671）
...
TIKZ = {'image1.png': ('figs/g1-prism.tikz', 41.1),      # 探二例1 直三棱柱（素材实测 41.099×68.936mm）
        ...
        'image3.png': ('figs/g3-cube6.tikz', 84.0),      # 探六例1 正方体（86.0×82.0 → 栏宽封顶 84.0，缩 0.9767）
        'image4.png': ('figs/g4-dihedral.tikz', 66.3),   # 探八例1 二面角（66.301×35.673）
        'image5.png': ('figs/g5-fold.tikz', 54.7),       # 探九例1 折叠矩形（自然外框 56.564×38.714 → 54.7，缩 0.9671）
        'sub3_B_4.png': ('figs/g6-triple.tikz', 84.0)}   # 条目3 投影三联（86.0×22.842 → 满栏 84.0，缩 0.9767）
...
def under_center_row(name, tag):
    ...
    log(f'3d-A. 片G 图下置居中（{tag}／{name}）：{path}，置宽 {box}（源尺寸档；素材 standalone 实测同比缩），'
...
#      片E 0909c（#40 图文绕排回流）：图旁文字仍走本几何档（并排 minipage 形不动），
#      图底以下文字按 split 标记切出、作独立段按通栏（\linewidth）排——几何三窗逐字不回退。
#      实测：\parshape/\hangindent 逐行设宽会触发 xeCJK 断点结构改变（探针 片E/probe_trace：
#      断点 14→35，图旁行右缘 180.3pt＝墨缝 2.62mm 出窗 5.2–8.2），故弃逐行设宽、取文本切分。 ----
COLW_MM = 84.0            # 栏宽（H1 片 0909c：qp-layout 版心 175.6mm − columnsep 7.6mm 折半＝84.0；旧 82.8/9.25）
SIDE_BOX_W_MM = 33.12     # 旧置入盒宽 0.40\linewidth（ink_w=None 时按此盒折算维持现行墨宽）
SIDE_TEXT_W_MM = 46.368   # 图旁窄栏默认宽（0.56 栏）
...
def side_row(text, tok, tag):
    r"""片E 0909c（#40）图文绕图回流（全品式）：图旁文字窄栏＋图底以下文字恢复通栏。
    结构＝左 \begin{minipage}[t]{T mm}\raggedright 图旁前缀 ＋ \hspace{G} ＋ 右
    \begin{minipage}[t]{PRINTW mm} 图盒（几何档 3c 与修复轮逐字一致）；图底以下余段由
    SIDE_DEF['split'] 标记切出，紧随其后作独立段按通栏排（\linewidth）。
    G = 栏宽 − r_edge + 图盒右留白 − 图旁栏宽 T − 图盒宽；图盒宽＝墨宽目标折算（剪垫不放大墨幅）。
    竖直：\raisebox 声明盒 [height=H][depth=\height−H]，H = c0 + pad_t + 0.25——图墨顶＝基线−H+pad_t、
    首行墨顶＝基线−c0，差落 [−1,+1]mm（目标 −0.25）。行高＝max(前缀, 图盒)：前缀短于图高时行高
    ＝图盒高（余段仍落图底之下）；前缀长于图高时余段落前缀之下（亦在图底之下）。
    空位括号右挂（探八）保留——挂在余段末行，通栏后右挂件落栏右（全品 p07 题1-3 形）。"""
    figs_seen.append(('side', tok, tag))
    hang = ''
    for tail_s in ('(~~~~)', '（）', '()'):
        if text.endswith(tail_s):
            text = text[:-len(tail_s)]
            hang = r'\nobreak\mbox{}\nobreak\hfill(\kongwei)\hspace{0.56mm}'
            log(f'3b-F. 绕图回流空位右挂 ×1（{tag}）：题干尾空位 {tail_s} 改 \\hfill 末行右挂＋尾胶 0.56mm'
                f'（全品 p07 题1-3 形；末段通栏后右挂件落栏右，孤括号行缺陷位消解）')
            break
    ...
    assert glue > 0 and text_w + glue + print_w <= COLW_MM + 0.02, \
        f'{name} 绕图几何出栏：glue={glue:.2f} total={text_w + glue + print_w:.2f}/{COLW_MM}'
    assert H > 0
    ...
    if rest:
        SIDE_SPLIT.append((name, len(text), len(rest)))
        log(f'3c-B. 绕图回流切分（{tag}／{name}）：图旁前缀 {len(text)} 字（窄栏断行同旧全窄形）＋'
...
def reflow(blocks_in, tag):
    ...
    for b in blocks_in:
        ...
        for mm in FIGRE.finditer(b):
            ...
            if kind == 'under':
                # 片G 0910：图下置居中（源尺寸档）——图 token 就地替换为矢量片段独立行，前后文字各自成块
                out.append(under_center_row(name, tag))
                last = mm.end()
                continue
...
def kbtable(h1, h2, h3, rows, widths, tcsep=3):
    ...
    n_tab[0] += 1
    usable = 84.0 - 2.4 * 25.4 / 72 - 6 * tcsep * 25.4 / 72   # H1：栏宽 84.0（E7）−框线 2.4pt−tabcolsep 6×tcsep，折 mm
...
# F 片F 0909c #41（行内拉伸治理）：2em→1em——逐档实测（rel=500 固定）overfull 0/0.25/0.5/0.75/1.0em
#   ＝12/4/2/1/0，取最小 overfull=0 档 1em。qp-layout 序言值同步 1em。
...
], ('10', '37.6', '29.2'))   # F：E 手改②上移——表2 末列 32→28.6mm；H1（E7 联动）：栏宽 84.0 → 二/三列 +1.2（总 84.0）
...
# 片G 0910：三联图改下置独立行后可能落条目末段——尾距挂载点改「末**文字**段」（图段 \input{figs/ 不参与），
# 否则 \tiaomutail 会错挂到图尾（条目 3→4 缝失去 0.35mm 标定档）。
_tail = [i for i in range(_n_emit0, len(body))
         if body[i].startswith(r'\bindp') and r'\input{figs/' not in body[i]]
assert _tail, '条目3 续段异常（无 \\bindp 文字段）'
body[_tail[-1]] += r'\tiaomutail'
...
], ('9.5', '37.1', '30.2'))   # F：E 手改②上移——表3 末列 33→29.6mm；H1（E7 联动）：栏宽 84.0 → 二/三列 +1.2（总 84.0）
...
def cebiao(nd, kn):
    ...
    # H1 片 0909c（E2 banjiao 连带）：前隙 2.7→4.1mm——检测题号「N．」的「．」是句号类标点，
    # banjiao 压缩后其盒后退 ~0.9–1.9mm（实测 5 处 [ ink＝1.76/0.74/1.46/1.46/1.77mm）；
    # 回填 1.4mm 使墨隙回 2.1–3.4 档（例/变式标签尾为数字不走 banjiao，qp-blocks 侧 2.7mm 不动）。
    return (r'\hspace{4.1mm}\tieside{' + nd + '（知识点' + kn + '）}' + r'\hspace{2.2mm}')
...
# 注释行与收盒 } 同块发射（块间 \n\n 会在两者间垫空行，与 E 定稿结构不符）
# H1 片 0909c 复标（E7 密度连带）：-9.7→-14.8pt——C 轮值按 10.09/18.25 密度标定；新密度 18.0 行距下
# 收尾页 Output \vbox 超 5.06pt（main.log「Overfull \vbox while \output active [6]」），负胶回填 5.1pt 消 overfull。
em(r'\vspace{-14.8pt}%@@VBOXGLUE@@' + '\n}')
...
#   （Times 连字符 w13×h4、两侧隙 4/4px，探针 m1/m2 同尺实测）。断点档 penalty100：
...
_strh_pat = [
    (r'ABC - A_{1}B_{1}C_{1}', r'ABC\text{-}\penalty100 A_{1}B_{1}C_{1}', 1),
    (r'ABCD-A_1B_1C_1D_1', r'ABCD\text{-}\penalty100 A_1B_1C_1D_1', 3),
    (r'ABCD - A_{1}B_{1}C_{1}D_{1}', r'ABCD\text{-}\penalty100 A_{1}B_{1}C_{1}D_{1}', 1),
    (r'A-EF-D', r'A\text{-}\penalty100 EF\text{-}\penalty100 D', 1),
    (r'B-AC-D', r'B\text{-}\penalty100 AC\text{-}\penalty100 D', 1),
]
...
log('6a-H2. E3 字母串连字符短横化（片H2 0909c）：ABC-A₁B₁C₁×1／ABCD-A₁B₁C₁D₁×3／'
    'ABCD - A₁B₁C₁D₁×1／A-EF-D×1／B-AC-D×1 → \\text{-}（清后原字面 0；断点 penalty100）')
...
# R2 绑定（防孤字断行）：①则＋行内公式 21 处禁断（则不落行尾）；②「是(\kongwei)」4 处禁断；
# ③探三题干 \emph{x}/\emph{y} 与 =空槽 绑定（y 孤字缺陷位）；④探八题干裸 ()→(\kongwei)（空位补齐）
# 片G 0910 适配：探八题干图由并排绕图改下置，side_row 退场 ⇒ 其「题干尾空位 \hfill 右挂件」不再产生，
# 裸 () 余量由 0 变 1——就地补 v4.4④ 定宽空位档（6.8mm），并并入 R2② 禁断绑定（防孤括号落行首）；
# kongwei 总数维持 9（旧＝右挂件 1，新＝内联 1），右挂件计数随之下线（n_hang 1→0）。
n_kw8 = body_text.count('()')
assert n_kw8 == 1 and body_text.count('距离是()') == 1, \
    f'R2 裸() 计数异常：{n_kw8}/1（片G 0910：探八例1 裸() 未被 side_row 消费，应恰 1 且在题干位）'
body_text = body_text.replace('()', r'(\kongwei)')
...
assert n_shi == 4, f'R2 是(\\kongwei) 计数异常：{n_shi}/4（3 源空位＋片G 探八补 1）'
...
n_kw8 = body_text.count('()')
assert n_kw8 == 0, f'R2 裸() 残留异常：{n_kw8}/0（片G 0910：探八裸() 已就地转 \\kongwei，此处应清 0）'
assert ('()' not in body_text) and ('(~~~~)' not in body_text), '空位杂形残留'
# R2⑤（F 收尾轮 V7d 设；片G 0910 下线）：并排题干空位末行右挂件随 side_row 退场——探八题干恢复通栏，
# 空位改走内联 (\kongwei) 定宽档（旧 1 → 新 0），孤括号行缺陷位由 R2② \nobreak 禁断承担。
n_hang = body_text.count('\\nobreak\\hfill(\\kongwei)')
assert n_hang == 0, f'R2⑤ 并排题干空位右挂计数异常：{n_hang}/0（片G 0910 side_row 退场，右挂件下线）'
log(f'6a-F. F 终审 pass：R1 定界符 \\left(/\\right)/\\left|/\\right|→普通 {n_lp}/{n_rp}/{n_lv}/{n_rv}、'
    ...
    f'R2 绑定 则\\nobreak\\( ×{n_ze}（21 行 23 处）、是\\nobreak(\\kongwei) ×{n_shi}（片G 0910：3 源＋探八补 1）、'
    f'emph x/y\\nobreak= ×2、探八裸() 就地转 \\kongwei ×1（片G 0910：side_row 退场，右挂件不再消费，清后残留 {n_kw8}）；'
    f'并排题干空位右挂 \\nobreak\\hfill(\\kongwei) ×{n_hang}（片G 0910 下线：旧 1→新 0，探八题干通栏、孤括号行由 \\nobreak 禁断消解）；'
    'kongwei 总数 9 维持（v4.4④ 转换 8＋探八 1，口径由右挂改内联；§8 断言同步）')
...
log(f'6d. 课堂评价 vbox 尾胶上移（E 手改③）：\\vspace{{-14.8pt}}（H1 片复标 -9.7→-14.8 消 5.06pt Output \\vbox overfull）＋C 轮标定注释占位回填 {n_vbg}/1（R9 复核不压页脚）')

# ---- 6e. H3 片 0909c（E8 句末标点统一）：自撰文本句末「。」→「.」（全品统一「.」）----
#   全品真迹（p04 同尺）：句末统一「.」＋字距（墨点 w7px、前后隙 3/18px@14.176），无「。」空心圈；
...
log(f'8. 逻辑断言自检（数据面）：◆探究点 {n_tj}/9；例1 {n_li1}/9（数字 \\textbf 加重，探五带 \\duoxuan）；变式1 {n_bs}/9；【答案】行 {n_ans}/14；'
    ...
    f'片G 图下置居中 {n_under}/6（源尺寸档；位图引用取消 {n_gref}/0——旧 media/media/*.png 保留不删，仅取消引用）；'
...
log('8c. 片G 0910 六图下置集成台账（全件 6 图；替代 F 片A 并排／片E 绕图口径）：'
    '六图一律「图下置居中·源尺寸档」，内容换重绘 TikZ 矢量片段（figs/g1-prism…g6-triple.tikz，'
    '素材 _tmp片G素材 逐要素核对＋600dpi 叠加终检通过），旧位图仅取消引用（media/media/*.png 保留不删）；'
    '置宽：g1 探二 41.1／g2 探三 42.0／g3 探六 84.0（源 86.0 超栏封顶，缩 0.9767）／g4 探八 66.3／'
    'g5 探九 54.7（素材自然外框 56.564，缩 0.9671）／g6 条目3 三联 84.0 满栏（旧 60mm，缩 0.9767）；'
