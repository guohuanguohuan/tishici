...
# 缩档收口 0910（试法二·采用）：条目3 末段尾距 0.35→0.80mm（＋0.45）——⑦ 条目缝第 5 对
# （p2c2 条目3→4，β 降部墨底→号行带顶）实测 2.54 越窗下沿 0.16mm（窗 2.7–3.6；缩档连带，见简报【四】）。
# 试法一（g6 图行尾胶 −1.0→−0.8）已试并回退：⑦ 虽回 2.71 但破 ⑱「六处逐字节同式」形制门＋0.01mm 无垫。
# 本法把胶加在**被测缝内部**（改胶点＝缝自身）：探针实证该缝带 glueguard 弹性再分配（试法一 +0.20 胶
# 使号行多移 +0.18），缝内刚体 +0.45 任凭弹性 ±0.2 读数仍落 2.8–3.2，窗内且远离下沿。
# 逐点微调不走 qp-blocks \\tiaomutail 全局档（全局 +0.45 会把第 2 对 3.56 顶出上沿 3.6）；
# 展开形与 \\tiaomutail 同构（\\par\\addvspace{0.35mm}），仅值不同，qp-blocks 一字未动。
body[_tail[-1]] += r'\par\addvspace{0.80mm}'
log('5e. 知识点三：条目 1～2（原 6～7 重编号）选编＋挖空印答＋编注并段（微提醒同段并排）；'
    ...
    '条目尾距移至末段（如图③段末），条目 3→4 缝落普通行距档；'
    '缩档收口 0910：本条目末段尾距 0.35→0.80mm（+0.45，展开形同 \\tiaomutail、逐点不触全局档）——'
    '⑦ 第 5 对缝（p2c2 条目3→4）2.54 越窗下沿 0.16mm 的缩档连带收口，目标读数 ≈2.99；'
