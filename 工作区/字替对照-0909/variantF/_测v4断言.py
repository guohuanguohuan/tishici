...
# 片E 0909c（#40）：前缀仍此形（图旁窄栏），图底以下余段另作通栏段——⑱/⑱-2 形与窗不变，⑱-3 新设管余段。
...
for pno in range(1, n_pages + 1):
    ...
    for info in infos:
        ...
        # F 片C 0909 适配（旧→新）：邻行配对加同栏约束 bb[0] >= cl2-2——旧式仅 rct.x0−COLW 下界，
        # 本片改版后 image1 落 p3 右栏 y≈453（旧位 y≈133），左栏行被误配为「图带邻行」致 fsub 空
        # （Exception）；同栏约束后五组几何全落窗（顶 −0.25～−0.30／缝 5.29–7.03／右 1.76–2.86）。
        near = [bb for bb, tt in tlines
                if bb[0] >= cl2 - 2 and bb[0] >= rct.x0 - COLW and bb[2] <= rct.x0 + 5
                and bb[1] < rct.y1 and bb[3] > rct.y0
        ...
        if not (abs(topdiff) <= 1.0 and 5.2 <= seam <= 9.0 and 1.6 <= right_edge <= 3.6):
            geo_ok = False
geo_ok = geo_ok and len(geo_rows) == 5
check('⑱-2 墨级几何（顶差|·|≤1／墨缝 5.2–9.0／右缘 1.6–3.6 ×5）', geo_ok, geo_note +
...
# ---- ⑱-3 绕图回流：图底以下余段首行通栏（片E 0909c #40 新设；旧形＝整段窄栏，作废） ----
# 口径：side 行＝minipage 并排（图旁前缀）；其后若紧跟非 \bindp/\bindopt 元素＝切分余段（通栏独立段）。
# 渲染量：每组取同栏、墨顶 ≥ 图墨底 的首个文字行（同一视觉行 pieces 合并 bbox——右挂件计入），
# 右缘距栏右 ≤5mm 为通栏。切分组（image4/image5）必测；段全在图旁组（image1/2/3）无余段，
# 首个下邻行系下邻块（登记，不设门）。另设零侵入门：图带内不得有文字行 x 越入图墨 x 带。
PX2NAME = {691: 'image1', 764: 'image2', 521: 'image3', 788: 'image4', 1798: 'image5'}
SPLIT_IMGS = {'image4', 'image5'}
els_b = body.split('\n\n')
row_rest = {}
for i, e in enumerate(els_b):
    m_img = re.search(r'image(\d)\.png', e)
    if '\\begin{minipage}[t]' in e and m_img:
        nxt = els_b[i + 1] if i + 1 < len(els_b) else ''
        is_rest = bool(nxt) and not re.match(r'\\(bindp|bindopt|tjdnr|liB|ansline|jiexi|xiaojie|zsd|zhenti|vbox|huaxing)', nxt)
        row_rest['image' + m_img.group(1)] = nxt if is_rest else ''
n_split_body = sum(1 for v in row_rest.values() if v)
rest_ok = n_split_body == 2 and all(row_rest.get(k) for k in SPLIT_IMGS)
tail_rows, tail_ok = [], rest_ok
for pno in range(1, n_pages + 1):
    page = doc[pno - 1]
    infos = page.get_image_info(xrefs=True)
    if not infos:
    ...
    for info in infos:
        ...
        # 零侵入门：图带内文字行不得 x 越入图墨 x 带
        intr = [bb for bb, _ in tlines if bb.x0 >= cl2 - 2 and bb.x0 < ink_l and bb.x1 > ink_l + 1
                and bb.y0 < ink_bot - 1 and bb.y1 > rct.y0]
        if intr:
            tail_ok = False
            print(f'   (⑱-3)侵入行 p{pno} {nm}：{intr[0]}')
        # 图底以下首个文字行（同视觉行合并）
        # H2 片 0909c 口径加严：先按 y 聚「视觉行」，取首个「自栏左起」（行内最左 piece x0 ≤ 栏左+5pt）的行——
        # 原口径把图旁窄栏行的尾段碎片（bbox 下探过图墨底 1–2pt，如 image5 的「2，」x0=380）误判为图下首行
        # （image5 假 51.69mm）；加严后行内右挂件（如选项（ ））仍计入同一视觉行（image4 仍取到 285.3 右挂）。
        _below_all = sorted([bb for bb, _ in tlines if bb.x0 >= cl2 - 2 and bb.x1 <= cl2 + COLW + 2 and bb.y0 >= ink_bot - 1],
                            key=lambda bb: bb.y0)
        _rows = []
        for _bb in _below_all:
            if _rows and _bb.y0 - _rows[-1][0] < 3.0:
                _rows[-1][1].append(_bb)
            else:
                _rows.append((_bb.y0, [_bb]))
        _first = next((r for r in _rows if min(b.x0 for b in r[1]) <= cl2 + 5.0), None)
        below = _first[1] if _first else []
        if not below:
            tail_rows.append((nm, None))
            if nm in SPLIT_IMGS:
                tail_ok = False
            continue
        y0f = below[0].y0
        ...
        redge = (cl2 + COLW - xr) / PT
        tail_rows.append((nm, redge))
        if nm in SPLIT_IMGS and redge > 5.0:
            tail_ok = False
check('⑱-3 绕图回流：切分组图底以下余段首行通栏（右缘 ≤5mm）＋图带零侵入（片E 0909c）',
      tail_ok and len(tail_rows) == 5,
      f'切分余段 {n_split_body}/2（{"image4/image5" if rest_ok else "异常"}）；' +
      '；'.join(f'{nm} 下首行右缘{("%.2fmm" % r) if r is not None else "无文字行"}' for nm, r in sorted(tail_rows)))
reg('⑱-3 口径登记（片E 0909c #40 新设）',
    '旧形（F 片A）：整段窄栏 minipage——图底以下文字仍走 46.4mm 窄栏（用户 #40 缺陷态）；'
    '新形：文本切分——图旁前缀窄栏（断行与旧全窄形逐行一致）＋余段通栏独立段（探八题干第 4 行起、'
    '探九详解第 5 行起）；探二/三/六段全在图旁（无余段，下首行系下邻块，登记）。'
    '弃 \\parshape/\\hangindent 逐行设宽：探针实证触发 xeCJK 断点结构改变（断点 14→35），'
    '图旁行右缘 180.3pt＝墨缝 2.62mm 出窗（片E/probe_trace.log）；切分法几何三窗逐字不回退。')
