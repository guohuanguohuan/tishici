...
for _pno in range(len(doc)):
    ...
    for _gy, _cs in _grp:
        ...
        for _i in range(len(_cs) - 1):
            ...
            if (_pno == 0 and abs(_gy - 488.9) < 0.6
...
reg('拉伸异常行口径（F 片F 0909c #41 新设；H3 片 0909c 加栏归属判）',
    ...
    '全品对齐0911：豁免 1 行 3 对——p1y488.9「在|的|直|线」3×5.51pt（0.53em）均拉，成因＝6h kongda 自适盒→'
...
check('㉑d 课前预习节值在场（三组填空整串＋判断答整串＋简析逐条首段≥5字锚·去空白包含判定）', d_ok,
...
reg('㉑d 口径（全品对齐0911 新立）', '册 body「一、课前预习」节每组 [答案]＝填空紧凑条目（N．值）＋判断 (N)√×'
    ...
    '过 _norm21 后 pdf 明文包含判定；简析锚＝首个 ≥5 字中数连串截 12（数学串 TeX↔pdf 形制不同、锚止于数学区前，㉑c run1 教训推广）；'
