# -*- coding: utf-8 -*-
r"""门-回流-课时19.py — M3 S3 W3臂G 课时19 练习件·回流门（零强制跳页）＋件型纪律断言。
（照抄母版门谱 工作区/_tmpM3S3母版0914/门谱/门-回流.py，PIECE/MANIFEST 常量换片；
逐片期望值：\duoxuan＝manifest 多选数 2（11/12）；\liubai＝解答题数 8（03~06 无小问 16mm×4
＋08 三问 32mm＋13/15/16 两问 24mm×3）；\kongda＝1（题10 填空 \kongda{2}）。）
用法: python 门-回流-课时19.py     退出码: 0＝全过；1＝有红。
"""
import io
import json
import os
...
MANIFEST = 'C:/提示词/工作区/M3-第2章量产0913/成卷/题面库/manifest/课时19.manifest.json'
...
check('\\liubai 计数＝解答题数 8（03~06/08/13/15/16；01/02/07/09/14 单选、10 填空、11/12 多选）',
      n_liubai == 8, f'{n_liubai}')
inband = all(16.0 <= float(h[:-2]) <= 40.0 for h in liubai_h)
check('\\liubai 逐题高度 16~40mm 带内（03~06 无小问 16mm×4＋08 三问 32mm＋13/15/16 两问 24mm×3）',
      inband and liubai_h == ['16mm', '16mm', '16mm', '16mm', '32mm', '24mm', '24mm', '24mm'],
      f'{liubai_h}')

n_kongda = len(re.findall(r'\\kongda(?![a-zA-Z])', body))
check('\\kongda 计数＝1（题10 填空印答档）', n_kongda == 1, f'{n_kongda}')
