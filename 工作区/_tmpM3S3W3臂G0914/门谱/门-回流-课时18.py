# -*- coding: utf-8 -*-
r"""门-回流-课时18.py — M3 S3 W3臂G 课时18 练习件·回流门（零强制跳页）＋件型纪律断言。
（照抄母版门谱 工作区/_tmpM3S3母版0914/门谱/门-回流.py，PIECE/MANIFEST 常量换片；
逐片期望值：\duoxuan＝manifest 多选数 1（02）；\liubai＝解答题数 13（03~09/11~16，全两问 24mm）；
\kongda＝0（本片填空 0）。）
用法: python 门-回流-课时18.py     退出码: 0＝全过；1＝有红。
"""
import io
import json
import os
...
check('\\liubai 计数＝解答题数 13（题面侧头标实录：01 单选/02 多选/10 单选，余 03~09/11~16 解答）',
      n_liubai == 13, f'{n_liubai}')
inband = all(16.0 <= float(h[:-2]) <= 40.0 for h in liubai_h)
check('\\liubai 逐题高度 16~40mm 带内（本片 13 题全两问 24mm 档）',
      inband and liubai_h == ['24mm'] * 13, f'{liubai_h}')

n_kongda = len(re.findall(r'\\kongda(?![a-zA-Z])', body))
check('\\kongda 计数＝0（本片填空 0）', n_kongda == 0, f'{n_kongda}')
