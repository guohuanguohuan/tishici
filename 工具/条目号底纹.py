...
#
"""条目号底纹.py — 条目题名行条目号「N．」＋条目第一子层「（N）」挂 C9C9C9 底纹（可复用，幂等）

口径注记（公共规则§7题号难度块底纹条款·条目号底纹＋条目第一子层底纹）：
  · 2026-08-29 用户拍板改口径＋同日成书形态拍板扩面：凡条目形内容的条目号「N．」挂
    w:shd val="clear" color="auto" fill="C9C9C9"——适用面＝知识清单件条目题名行＋衔接件讲部条目
  ...
  · 2026-08-29 成书形态拍板新增「条目第一子层底纹」：条目下第一子层「（N）」（全角括号）同款
    挂 C9C9C9、只盖「（N）」本身、不加粗；恒等式＝第一子层底纹 run 数＝第一子层计数。
    第二子层①②③、更深层复用（N）（隔层复用层）、题目小问半角(1)(2)与详解步骤序号不挂。
  · 题号块排斥（扩面必配）：题号块「N．（简单/中档/难）」（题号后带档位全角括号）不是条目，
    一律跳过——讲练件/衔接件题号流天然排除。退化真题卷件题号块「N．」无档位者与条目同形，
    本工具不适用该类件（不传入即可）。

判定与行为：
  · 条目题名行＝body 级段落全文匹配 ^\\d+． 且非题号块形态（^\\d+．（简单|中档|难））；
  ...
  · 已挂（恰有一独立 C9C9C9 run）→ 跳过并计数（幂等）；底纹 run 强制去加粗；
...
QBLOCK_RE = re.compile(r'^\d+．（(?:简单|中档|难)）')   # 题号块（题号＋档位全角括号）——排斥
...
def main(path, regmd):
    ...
    lines.append('C9C9C9、不加粗、只盖条目号/（N）本身；题号块「N．（档位）」排斥；第二子层①②后')
