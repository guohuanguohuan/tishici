...
用法: python 工具/知识清单图片恢复.py <源文件.docx> <待修成品.docx> <输出.docx>
输入输出: 取源docx章首4个导图段落（纯图复制，rId9 缩放86%消末页留白）逐段插到成品「12.1 电路中的能量转化」标题之后，另存为输出docx；硬校验＝源恰4个图引用、成品恰7个图引用、两件相同rId指向同一图片否则拒绝迁移（仅适配物理必修3第12章知识清单）。"""
