# -*- coding: utf-8 -*-
"""解析块浅底挂载.py — 2026-09-01 A'改制轮（T4·工具债④，口径D／公共规则§7解析块段落底纹款·第七类底纹）。
带题件（讲练件/衔接件/实验卷/真题卷等）解析块全部段落铺段落级 w:shd val="clear" color="auto" fill="F2F2F2"
（≈5%灰、PDF灰度≈242），挂 pPr（段落级），与 run 级标记（题号块/块标签芯片/文字型答案值 C9C9C9）分层并存。
  适用段（解析块段落）：
    ①题块内【答案】【知识点】【分析】【详解】【点睛】【编注】【大招指引】【题后反思】【温馨提醒】
      标签行及其后全部续段（自题块内首个解析标签起至题块末，含答案跨段续值段/图段/空段）；
    ②题型通式句：题型组标题之后、首题题号之前的【编注】（题型通式）段落（按最近标题＝题型组判定）。
  白底维持（不铺）：题干/选项/小问（题块内首个解析标签之前）、讲部讲解正文、讲部/清单条目（含其【编注】
    说明句——讲部/条目/节语境的标签行一律白底并登记）、标题、导航表；表格内段落不铺（解析态中途遇表格登记待核）。
  知识清单件（文件名含「知识清单」）整件跳过（清单无题干/解析之分，全件白底）——文件不触碰、不重写。
  恒等式＝浅底段落数＝解析块段落清点数（输出分类计数＋逐题块明细供人工对照）。
  幂等：已 F2F2F2 段落不重复挂载；零破坏断言＝run级 shd 各 fill 计数前后恒等＋w:t/m:t 文字流前后恒等
    ＋标题段（段级 ADC2DA/C6D4E3）零触碰。
  块边界识别参照 工具/extract_structure.py 解析逻辑（题号块签名＋块内含【答案】），题号形态兼容全局「N．」
    与层级制「节号-序号．」（A'改制轮口径C前后两态可用）。
...
def process(path, args):
    ...
    # —— 解析段落清点 ——
    scope = {}      # idx -> (类别, 题块序号；通式句=-1)
    cat = Counter()
    detail = []     # (题块序号, 题号文本, 解析段数, 首标签)
    skip_labels_out = 0
    tbl_mid = 0
    for bi, (s, e) in enumerate(blocks):
        ...
        for k in range(s + 1, e):
            ...
            if el.tag != q('p'):
                if state is not None:
                    tbl_mid += 1
                continue
            lb = lead_label(ptexts[k])
            if lb:
                state = lb
                if first_lb is None:
                    first_lb = lb
                key = '答案行' if lb == '【答案】' else ('知识点行' if lb == '【知识点】'
                                                       else lb.strip('【】') + '块')
                scope[k] = (key, bi)
                cat[key] += 1
                n_blk += 1
        ...
        if first_lb is not None:
            detail.append((bi, ptexts[s][:24], n_blk, first_lb))
    # 题型通式句：题块外、【×】起段、最近标题＝题型组（题型标题后/首题前，或组尾散注）
    cur_head = None
    for i, el in enumerate(els):
        if el.tag != q('p'):
            continue
    ...
    # —— 恒等式与断言 ——
    total_f2 = sum(1 for el in els if para_fill(el) == FILL)   # dry-run 时＝改写前实存值
    stray = [i for i, el in enumerate(els)
             if para_fill(el) == FILL and i not in scope]
    hit_title = sum(1 for i in scope if para_fill(els[i]) in (FILL_TITLE1, FILL_TITLE2))
    lines = ['◆ %s' % basename]
    lines.append('  题块 %d｜解析块段落清点 %d（新挂 %d＋已有 %d；fill=auto 原位改写 %d%s）'
                 % (len(blocks), predicted, newly, already, replaced_auto,
                    '，dry-run 未写回' if args.dry_run else ''))
    lines.append('  分类：%s' % '；'.join('%s%d' % kv for kv in sorted(cat.items()) if kv[1]))
    if args.dry_run:
        lines.append('  恒等式（写回后应为）：浅底段落数 %d ＝ 解析块段落清点数 %d（dry-run 预演）'
                     % (already + newly, predicted))
    else:
        lines.append('  恒等式：浅底段落数（实存 pPr fill=F2F2F2）%d ＝ 解析块段落清点数 %d %s'
                     % (total_f2, predicted, '✓' if total_f2 == predicted else '←≠'))
    if stray:
        lines.append('  ! 范围外已存在浅底段落 %d 个（body序号样本 %s）——请人工核对归属'
                     % (len(stray), stray[:6]))
    if skip_labels_out:
        lines.append('  登记：题块外标签行 %d 段维持白底（讲部/条目/节语境——清单【编注】说明句副本等）'
                     % skip_labels_out)
    if tbl_mid:
        lines.append('  ! 解析态中途遇表格 %d 处（表格内段落未铺浅底，登记待人工核验）' % tbl_mid)
    if hit_title:
    ...
    if args.detail and detail:
        ...
        for bi, qt, cnt, lb in detail[:args.detail]:
            lines.append('    题块%02d %s 解析段%d 首标签%s' % (bi, qt, cnt, lb))
...
def main():
    ...
    for p in args.files:
        ...
        if '知识清单' in os.path.basename(p):
            out.append('◆ %s：整件跳过（知识清单——全件白底不铺解析块浅底，文件未触碰）'
                       % os.path.basename(p))
            continue
        z = zipfile.ZipFile(p)
