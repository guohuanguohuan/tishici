...
"""灰度四值校验.py（原名 灰度三值校验.py，2026-09-01 A'改制轮工具债⑦升四值更名——
  ...
  四值：#ADC2DA→灰≈190（章/节标题整行底纹）、#C6D4E3→≈209（讲部/题型）、
        #C9C9C9→201（内容标记族）、#E0E0E0→224（题干底纹，A''口径——原F2F2F2解析浅底242废止），
  各±容差（默认±8；201/209两带相距8互有重叠——分色以矢量层为主、像素带为辅，三重定性沿用
  2026-08-29 全库改色抽验先例：矢量层＋144dpi平台直方图；224带与白底255距离31、带外不误吞；224与201相距23分带清晰）。
  取样两路（兼容矢量PDF与渲染平台）：
...
TARGETS = [  # (名称, hex, R,G,B, 设计灰)
    ...
    ('题干底纹#E0E0E0', 'E0E0E0', 224, 224, 224, 224),
]
...
def main():
    ...
    L.append('结论: 矢量四值命中 %s｜矢量离群 %d 色｜像素簇 %d 个（三重定性：矢量层为主，'
             '像素带/簇为辅；带互有重叠——201/209相距8；224距白底255为31）'
