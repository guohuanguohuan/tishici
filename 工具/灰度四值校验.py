...
"""灰度四值校验.py（原名 灰度三值校验.py，2026-09-01 A'改制轮工具债⑦升四值更名——
  ...
  四值：#ADC2DA→灰≈190（章/节标题整行底纹）、#C6D4E3→≈209（讲部/题型）、
        #C9C9C9→201（内容标记族）、#F2F2F2→242（题干底纹，2026-09-05口径——A''过渡值#E0E0E0/224废止），
  各±容差（默认±8；201/209两带相距8互有重叠——分色以矢量层为主、像素带为辅，三重定性沿用
  2026-08-29 全库改色抽验先例：矢量层＋144dpi平台直方图；242带距白底255仅13、带外边界须与离群簇判读合并看；242与201相距41分带更宽）。
...
def main():
    ...
    L.append('结论: 矢量四值命中 %s｜矢量离群 %d 色｜像素簇 %d 个（三重定性：矢量层为主，'
             '像素带/簇为辅；带互有重叠——201/209相距8；242距白底255仅13）'
